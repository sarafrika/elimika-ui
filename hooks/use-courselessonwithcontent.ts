...
import { getAllContentTypesOptions, getCourseLessonsOptions, getLessonContentOptions } from '../services/client/@tanstack/react-query.gen';
...
export function useCourseLessonsWithContent({ courseUuid }: Params) {
    ...
    const contentTypeMap = useMemo(() => {
        return Object.fromEntries(
            contentTypeData.map((ct) => [ct.uuid, ct.name.toLowerCase()])
        );
    }, [contentTypeData]);
