...
const badgeClasses: Record<TrainingHubManagedCourse['accent'], string> = {
  ...
  yellow:
    'bg-[color-mix(in_srgb,var(--warning)_70%,white)] text-[color-mix(in_srgb,var(--warning)_92%,black)]',
};
...
export function ManageCourseCard({ course }: ManageCourseCardProps) {
  ...
  return (
    <article className='border-border/60 bg-card w-full max-w-full min-w-0 overflow-hidden rounded-[12px] border p-2.5 shadow-sm sm:p-3'>
      <div className='flex w-full min-w-0 gap-3 py-1.5'>
        <div className='min-w-0 flex-1 basis-0'>
          ...
          >
            <div className='min-w-0'>
              <h3 className='text-foreground group-hover:text-primary block w-full min-w-0 truncate overflow-hidden text-[0.98rem] font-semibold sm:text-[1rem]'>
                {course?.title}
              </h3>
            </div>

            {/* META */}
            <div className='text-muted-foreground mt-1 flex w-full min-w-0 gap-x-2 text-[0.77rem] sm:text-[0.8rem]'>
              <span className='text-primary min-w-0 truncate font-medium'>{course.provider}</span>
            ...
            {/* STATUS */}
            <div className='text-success mt-1 flex min-w-0 items-center gap-1 text-[0.74rem]'>
              <BadgeCheck className='size-3.5 shrink-0' />
              <span className='min-w-0 truncate'>Admin verified • Approved to train</span>
            </div>
          </Link>

          {/* STATS */}
          <div className='text-muted-foreground mt-3 flex min-w-0 gap-4 text-[0.77rem] sm:text-[0.8rem]'>
        ...
        <div
          aria-hidden='true'
          className={`border-border/60 relative hidden h-[74px] w-[86px] shrink-0 overflow-hidden rounded-[10px] border sm:block ${accentClasses[course.accent]}`}
        >
          {imageUrl ? (
            <Image
              src={imageUrl}
