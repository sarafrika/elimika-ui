...
export function RubricGradingMatrix({
  ...
  const sortedCriteria = useMemo(
    () => [...matrix.criteria].sort((a, b) => (a.display_order ?? 0) - (b.display_order ?? 0)),
    [matrix.criteria]
  );
  ...
  const { totalScore, maxScore, percentage, isPassing } = useMemo(() => {
    ...
    sortedCriteria,
    sortedLevels,
    matrix.rubric.min_passing_score,
    matrix.rubric.max_score,
  ]);
  ...
  return (
    <div className='space-y-3'>
      {/* ── Score summary ── */}
      <div className='border-border/70 bg-muted/30 flex flex-wrap items-center justify-between gap-2 rounded-md border px-3 py-2'>
        ...
        {allRowsGraded ? (
          ...
          <span className='text-muted-foreground text-[11px]'>
            {sortedCriteria.filter(c => selections[c.uuid]).length}/{sortedCriteria.length} criteria
            graded
          </span>
        )}
      </div>
          ...
          <table className='min-w-max border-collapse text-left'>
            <thead>
              <tr className='bg-muted/40'>
                ...
                {sortedLevels.map(level => (
                  <th
                    key={level.uuid}
                    className='border-border/70 text-foreground border-b px-3 py-2 text-xs font-semibold'
                  >
            ...
            <tbody>
              {sortedCriteria.map((criteria, rowIdx) => {
                ...
                return (
                  <tr key={criteria.uuid} className={rowIdx % 2 === 1 ? 'bg-muted/10' : undefined}>
                    <td className='border-border/50 bg-background sticky left-0 z-10 min-w-[180px] border-b px-3 py-2 align-top'>
                    ...
                    {sortedLevels.map(level => {
                      ...
                      return (
                        <td
                          key={level.uuid}
                          className='border-border/50 border-b px-2 py-2 align-top'
                        >
                          <button
                            type='button'
                            disabled={readOnly}
