...
import { Lesson } from "@/app/dashboard/instructor/course-management/create-new-course/_components/lesson-management-form"
...
export async function fetchCourseCategories(params?: string) {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/categories${params ? `?${params}` : ""}`,
      { headers },
    )
...
export async function createLesson(
  ...
  try {
    ...
    throw new Error(
      "Something went wrong while creating lesson. Please contact support.",
    )
  }
}
...
export async function fetchCourses(params?: string) {
  try {
    ...
    const response = await fetch(
      `${BASE_URL}/courses${params ? `?${params}` : ""}`,
      { headers },
    )
    ...
    throw new Error(
      "Something went wrong while fetching courses. Please contact support.",
    )
  }
}
