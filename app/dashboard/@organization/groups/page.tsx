      ...
      <DialogContent className="max-w-lg">
        ...
        <form
          ...
          onSubmit={e => {
            ...
            const description = (form.elements.namedItem('g-desc') as HTMLTextAreaElement)?.value.trim();
            if (!name) {
              toast.error('Group name is required.');
              return;
            }
            create.mutate(
              { path: { organisationUuid }, body: { name, group_type: groupType || undefined, description: description || undefined } },
              {
                onSuccess: async () => {
                  setOpen(false);
...
function AddMembersDialog({
  ...
  const students = extractPage<User>(studentsQuery.data).items.filter(s => s.uuid && !existing.has(s.uuid));
      ...
      <DialogContent className="max-w-lg">
        ...
        <ScrollArea className="h-[320px] pr-3">
          {studentsQuery.isLoading ? (
            ...
            <p className="py-8 text-center text-sm text-muted-foreground">No more students to add.</p>
          ) : (
            <div className="space-y-1">
              {students.map(s => (
                ...
                >
                  <input type="checkbox" className="h-4 w-4 accent-primary" checked={selected.includes(s.uuid)} onChange={() => toggle(s.uuid)} />
                  <Avatar className="h-7 w-7">
                    <AvatarFallback className="bg-primary/10 text-primary text-[10px] font-semibold">{initials(s)}</AvatarFallback>
                  </Avatar>
              ...
              add.mutate(
                { path: { groupUuid }, body: { student_uuids: selected } },
                {
                  onSuccess: async () => {
                    ...
                    await qc.invalidateQueries({ queryKey: listMembersQueryKey({ path: { groupUuid } }) });
                    await qc.invalidateQueries({ queryKey: listGroupsQueryKey({ path: { organisationUuid } }) });
                  },
...
function GroupMembers({ organisationUuid, groupUuid, studentsByUuid }: { organisationUuid: string; groupUuid: string; studentsByUuid: Map<string, User> }) {
  const qc = useQueryClient();
  ...
  if (members.length === 0) {
    return <p className="py-4 text-center text-sm text-muted-foreground">No students in this group yet.</p>;
  }
  return (
    <div className="space-y-1">
      {members.map(m => {
        ...
        return (
          <div key={m.uuid} className="flex items-center gap-3 rounded-md border p-2">
            <Avatar className="h-7 w-7">
              <AvatarFallback className="bg-primary/10 text-primary text-[10px] font-semibold">{initials(student)}</AvatarFallback>
            </Avatar>
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-6 px-3 py-4 sm:px-5 lg:px-6 2xl:max-w-[1840px]">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Card className="border-l-4 border-l-success">
          <CardContent className="p-6">
            ...
            <div className="text-xs text-muted-foreground">Grouped students</div>
          </CardContent>
        </Card>
      ...
      {groupsQuery.isLoading ? (
        ...
        <EmptyState icon={Layers} title="No groups yet" description="Create a group to organise students into cohorts or streams." />
      ) : (
          ...
          {groups.map(g => (
            <AccordionItem key={g.uuid} value={g.uuid as string} className="rounded-lg border px-4">
                  ...
                  <div className="flex min-w-0 items-center gap-3 text-left">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {g.group_type && <Badge variant="outline" className="text-[10px]">{g.group_type}</Badge>}
                        <Badge variant="secondary" className="text-[10px]">{Number(g.member_count ?? 0)} member{Number(g.member_count ?? 0) === 1 ? '' : 's'}</Badge>
                      </div>
              ...
              <AccordionContent className="space-y-3 pb-4">
                ...
                <GroupMembers organisationUuid={organisationUuid} groupUuid={g.uuid as string} studentsByUuid={studentsByUuid} />
              </AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
