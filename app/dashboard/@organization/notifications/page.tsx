...
const isReminder = (n: NotificationDto) => {
  ...
  return s.includes('remind') || s.includes('due') || s.includes('schedul') || s.includes('deadline');
};

const priorityTone = (n: NotificationDto) => {
  const p = (n.priority ?? '').toLowerCase();
  if (p.includes('high') || p.includes('urgent') || p.includes('critical')) return 'bg-destructive/10 text-destructive';
    ...
    >
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Compose a message to a specific audience. Send now or schedule for later.</DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="title" value={title} onChange={e => setTitle(e.target.value)} placeholder="e.g. Term 2 schedule update" maxLength={120} />
          ...
          <div className="space-y-1.5">
            ...
            <Textarea id="message" value={message} onChange={e => setMessage(e.target.value)} placeholder="Write the details recipients will see…" maxLength={1000} rows={5} />
            <div className="flex items-center justify-between">
          ...
          <div className="space-y-2 rounded-lg border bg-muted/30 p-3">
            <label className="flex items-center gap-2 text-sm font-medium">
              <input type="checkbox" checked={schedule} onChange={e => setSchedule(e.target.checked)} className="h-4 w-4 rounded border-input accent-primary" />
              Schedule for later
            </label>
            {schedule && (
              <div className="space-y-1.5">
                <Popover>
                  <PopoverTrigger asChild>
                    <Button type="button" variant="outline" className={cn('w-full justify-start text-left font-normal', !scheduledAt && 'text-muted-foreground')}>
                      <CalendarIcon className="mr-2 h-4 w-4" />
                      {scheduledAt ? format(scheduledAt, 'PPP') : 'Pick a date'}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={scheduledAt} onSelect={setScheduledAt} disabled={d => d < new Date(new Date().setHours(0, 0, 0, 0))} initialFocus className={cn('p-3 pointer-events-auto')} />
                  </PopoverContent>
                </Popover>
...
function NotificationCard({ n }: { n: NotificationDto }) {
  ...
  return (
    <div className={cn('rounded-lg border bg-card p-3', unread && 'border-primary/30')}>
      <div className="flex items-start gap-3">
        <div className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-full', priorityTone(n))}>
          <Icon className="h-4 w-4" />
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <p className="text-sm font-semibold leading-tight">{n.title ?? 'Notification'}</p>
          ...
          <div className="mt-2 flex items-center gap-3">
            <span className="text-[11px] text-muted-foreground">{when ? dayjs(when).fromNow() : '—'}</span>
            {n.action_url && (
...
export default function NotificationsPage() {
  ...
  const filtered = useMemo(
    () => notifications.filter(n => (tab === 'all' ? true : tab === 'reminders' ? isReminder(n) : !isReminder(n))),
    [notifications, tab]
  );
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <CardTitle className="text-base">
                Inbox
                {unreadCount > 0 && <span className="ml-2 text-xs font-normal text-muted-foreground">{unreadCount} unread</span>}
              </CardTitle>
          ...
          <CardContent className="space-y-2">
            {notificationsQuery.isLoading ? (
              ...
              <EmptyState icon={Bell} title="You're all caught up" description="New alerts and reminders will appear here." />
            ) : (
            ...
            <CardContent className="space-y-2 text-sm">
              <div className="flex items-center justify-between rounded-lg border p-3">
                <span className="flex items-center gap-2 text-muted-foreground">
                  <BellRing className="h-4 w-4 text-success" /> Alerts
                </span>
                <span className="font-semibold">{alertsCount}</span>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                <span className="flex items-center gap-2 text-muted-foreground">
                  <Clock className="h-4 w-4 text-warning" /> Reminders
                </span>
                <span className="font-semibold">{remindersCount}</span>
              </div>
              <div className="flex items-center justify-between rounded-lg border p-3">
                <span className="flex items-center gap-2 text-muted-foreground">
                  <CheckCheck className="h-4 w-4 text-primary" /> Unread
                </span>
