...
import { ClassFormValues, CreateClassDialog, EditClassDialog } from "../_components/class-management-form"
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <Button onClick={openCreateClassDialog} type="button" className="cursor-pointer px-4 py-2 text-sm" asChild>
          <div>
            ...
            {draftClasses.data.content.map((cls: any) => (
              <TableRow key={cls.uuid}>
                <TableCell className="font-medium">
                  <div>
                    <div>{cls.name}</div>
                    <div className="text-muted-foreground max-w-[250px] truncate text-sm">{cls.description}</div>
                  </div>
                </TableCell>
                <TableCell>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem asChild>
                        <div onClick={() => openEditClassDialog(cls.uuid)} className="flex w-full items-center">
                          <PenIcon className="mr-2 h-4 w-4" />
                          Edit
                        </div>
                      </DropdownMenuItem>
                      <DropdownMenuItem asChild>
                        <Link href={`/dashboard/classes/preview/1a2b3c`} className="flex w-full items-center">
