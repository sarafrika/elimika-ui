...
export default function CourseCreationPage() {
  ...
  // get course by ID
  const { data: courseData, refetch: refetchCourse } = tanstackClient.useQuery(
    'get',
    '/api/v1/courses/{uuid}',
    {
  ...
  // get lesson by Id
  const { data: lessonData } = tanstackClient.useQuery(
    'get',
    ...
    content:
      lesson && lessonContent
        ? lessonContent?.map((item: any) => {
            ...
            return {
              contentType: matchedType?.name.toUpperCase() as
                | 'AUDIO'
                | 'VIDEO'
                | 'TEXT'
                | 'LINK'
              ...
              uuid: item?.uuid || '',
              value:
                matchedType?.name.toUpperCase() === 'TEXT'
                  ? item?.value || ''
                  : item?.file_url || '',
              duration:
