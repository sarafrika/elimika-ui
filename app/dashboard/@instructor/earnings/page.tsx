...
import { Calendar, CreditCard, DollarSign, Download, ShoppingCart, TrendingUp } from 'lucide-react';
      ...
      <Tabs defaultValue='overview' className='space-y-6'>
        ...
        <TabsContent value='overview' className='space-y-6'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>Monthly revenue vs sales</CardDescription>
            </CardHeader>
            ...
            <CardContent>
              <ResponsiveContainer width='100%' height={320}>
                <LineChart data={revenueData}>
                  ...
                  <Line type='monotone' dataKey='sales' stroke='var(--chart-2)' strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
                ...
                <tbody>
                  {earningsData.recentTransactions.map(txn => (
                    <tr key={txn.id} className='hover:bg-muted/50 border-b'>
                      ...
                      <td className='p-3 font-medium'>{txn.course}</td>
                      <td className='text-muted-foreground p-3'>{txn.student}</td>
                      <td className='p-3 text-right font-semibold'>${txn.amount.toFixed(2)}</td>
                      <td className='p-3'>
                        <Badge variant={txn.status === 'cleared' ? 'default' : 'secondary'}>
                          {txn.status}
                        </Badge>
                      </td>
                    </tr>
        ...
        <TabsContent value='payouts'>
          <div className='grid gap-6 md:grid-cols-3'>
            <SummaryCard title='Total Withdrawn' value={`$${earningsData.withdrawn.toFixed(2)}`} />
            <SummaryCard
              title='Available Balance'
              value={`$${earningsData.availableBalance.toFixed(2)}`}
            />
