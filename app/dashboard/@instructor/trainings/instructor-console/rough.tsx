...
export default function ClassPreviewPage() {
  ...
  const featuredContent =
    lessonsWithContent && lessonsWithContent.length > 0 ? lessonsWithContent[0] : null;
  const featuredImage = '/placeholder.png';

  return (
    <div className='bg-background text-foreground flex min-h-screen'>
      {/* LEFT SIDEBAR — STUDENT LIST */}
      <aside className='bg-card w-72 border-r'>
        <div className='p-4'>
          <div className='relative'>
            <Search className='text-muted-foreground absolute top-2.5 left-3 h-4 w-4' />
            <Input type='text' placeholder='Search Student' className='pl-10' />
          </div>
        </div>
        ...
        <ScrollArea className='h-[calc(100vh-80px)] flex-1 px-2 py-1'>
          {roster?.map((entry: any, idx: number) => {
            ...
            return (
              <Button
                ...
                className={`mb-1 flex h-auto w-full items-center justify-between rounded-md px-3 py-2 text-sm transition-colors ${isSelected ? 'font-medium' : ''} `}
                onClick={() => {
              ...
              >
                <div className='flex items-center gap-3'>
                  {/* Avatar initials */}
                  <div className='bg-primary/20 text-primary grid h-8 w-8 place-items-center rounded-full text-xs'>
                    {name
                      ?.split(' ')
                      .map((n: string) => n?.[0])
                      .slice(0, 2)
          ...
          {roster?.length === 0 && (
            <div className='text-muted-foreground flex flex-col items-center justify-center py-10 text-center'>
              <Users className='mb-3 h-8 w-8 opacity-70' />
      ...
      <main className='flex-1 overflow-y-auto px-4 py-6'>
        <div className='bg-card rounded-xl border p-6 shadow-sm'>
          ...
          <div className='mb-6 flex items-start justify-between gap-6'>
            <div className='flex-1'>
              ...
              <p className='text-muted-foreground mt-1 text-sm'>
                Submission Date: <span className='text-foreground font-medium'>10/02/2024</span>
              </p>
            </div>
          ...
          <div className='mb-5 w-full overflow-hidden rounded-xl'>
            <img src={featuredImage} alt='Preview' className='h-64 w-full object-cover' />
          </div>

          {/* Description */}
          <p className='text-foreground/80 mb-6 text-sm leading-7'>
            {classData?.description || course?.description || 'Lorem ipsum…'}
          </p>

          {/* Secondary content */}
          <div className='bg-muted flex aspect-video items-center justify-center rounded-lg'>
            <button className='bg-primary hover:bg-primary/90 rounded-full px-4 py-3 text-white shadow'>
              ▶ Play
            </button>
          </div>
        </div>
      ...
      {selectedStudent && (
        <aside className='bg-card w-80 overflow-y-auto border-l p-6'>
          {/* Header */}
          ...
          {/* Submission Card */}
          <div className='mb-6'>
            <p className='text-muted-foreground mb-2 text-sm'>Submission</p>
            ...
            <div className='mb-4'>
              ...
              <RadioGroup
                value={status}
                onValueChange={value => setStatus(value as 'Submitted' | 'Excused' | 'Missing')}
          ...
          {/* Submission Details */}
          <div>
            <p className='text-muted-foreground mb-2 text-sm'>Submission Details</p>
            ...
            <div className='mb-3'>
              <p className='text-foreground mb-2 text-sm font-medium'>Files Uploaded</p>
              ...
              <ul className='space-y-2 text-sm'>
                ...
                {uploadedFiles.map((file, i) => (
                  <li
                    key={i}
                    className='text-primary flex cursor-pointer items-center gap-2 hover:underline'
                  >
            ...
            <Button
              variant='link'
              className='text-primary mt-3 flex items-center gap-2 text-sm font-medium'
            >
              <MessageCircle className='h-4 w-4' /> View Comments (50)
            </Button>
          </div>
        </aside>
      )}
