...
const Availability = () => {
  ...
  const { data: Iavailability, refetch } = useQuery(
  ...
  const updateSlot = (dayIndex: number, slotIndex: number, field: keyof Slot, value: string) => {
    const updated = [...availability];
    if (updated[dayIndex] && updated[dayIndex].slots && updated[dayIndex].slots[slotIndex]) {
  ...
  return (
    <main className='mb-20 space-y-6'>
      <Tabs defaultValue='calendar' className='space-y-4'>
        <TabsList className='w-full justify-start gap-2 lg:w-[70%]'>
          <TabsTrigger className='py-1.5' value='calendar'>
            Calendar Availability View
          </TabsTrigger>
          <TabsTrigger className='py-1.5' value='settings'>
              ...
              >
                ...
                <div className='mb-2 flex min-w-[150px] items-center justify-between'>
                  <Label className='flex items-center gap-2'>
                    <Switch checked={day.enabled} onCheckedChange={() => toggleDay(dayIndex)} />
