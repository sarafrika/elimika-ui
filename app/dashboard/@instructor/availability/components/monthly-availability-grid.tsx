...
export function MonthlyAvailabilityGrid({
    ...
    const getDayStatus = (date: Date) => {
        ...
        const dayClasses = classes.filter(classItem => {
            ...
            return classItem.timetable.timeSlots.some(timeSlot => {
                ...
                const isWithinPeriod = date >= new Date(classItem.academicPeriod.startDate) &&
        ...
        const bookedSlots = daySlots.filter(slot => slot.status === 'booked').length + dayClasses.length + dayEvents.length;
    ...
    const getDayClass = (date: Date, status: ReturnType<typeof getDayStatus>) => {
        ...
        let baseClasses = "h-20 p-2 border border-gray-200 rounded cursor-pointer transition-colors relative";
                                            ...
                                            {status.events.length > 0 && (
                                                <div className="absolute top-6 left-1 right-1">
                                                    {status.events.slice(0, 2).map((event, idx) => (
                                                        <div
                                                            key={event.id}
                                                            className="text-xs px-1 py-0.5 mb-1 rounded truncate bg-blue-100 text-blue-800 border border-blue-200"
                                                        >
                                        ...
                                        <div className="text-sm space-y-1">
                                            ...
                                            {status.available > 0 && (
                                                <div className="text-green-600">
                                                    ✓ {status.available} available slots
                                                </div>
                                            )}
                                            ...
                                            {status.events.length > 0 && (
                                                <div className="text-xs text-purple-600 mt-1">
                                                    Events: {status.events.map(e => `${e.title} (${e.startTime}-${e.endTime})`).join(', ')}
                                                </div>
                                            )}
            ...
            <Card className="p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div className="text-sm text-gray-600">
                        Click days to add or edit events
                    </div>
                </div>
            </Card>
