...
import { getInstructorAvailabilityOptions, getInstructorAvailabilityQueryKey, setInstructorWeeklyAvailabilityMutation } from '../../../../../services/client/@tanstack/react-query.gen';
...
export default function AvailabilityManager({
    ...
    const { data: availabilitySlots, refetch } = useQuery(getInstructorAvailabilityOptions({ path: { instructorUuid: user?.instructor?.uuid as string } }))
    ...
    const getStatusInfo = () => {
        ...
        const availableSlots = availabilityData?.slots?.filter(slot => slot.is_available === true).length;
        const blockedSlots = availabilityData?.slots?.filter(slot => slot.custom_pattern === "BLOCKED_TIME_SLOT").length;
        // update booked slots
        const bookedSlots = availabilityData?.slots?.filter(slot => slot.custom_pattern === 'booked').length;
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <Avatar className="w-12 h-12">
                                ...
                                <AvatarFallback>
                                    {user?.instructor?.full_name?.split(' ').map(n => n[0]).join('')}
                                </AvatarFallback>
                            </Avatar>
                            <div>
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">When someone books a slot</p>
                            </div>
                            <Switch />
                        </div>
                        <div className="flex items-center justify-between">
                    ...
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground">Prevent double booking</p>
                            </div>
                            <Switch />
                        </div>
                    </CardContent>
