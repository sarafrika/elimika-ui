...
import { createClassDefinitionMutation, createClassRecurrencePatternMutation, getAllActiveClassDefinitionsQueryKey, getAllCoursesOptions, getClassRecurrencePatternQueryKey, scheduleRecurringClassFromDefinitionMutation, updateClassDefinitionMutation, updateClassRecurrencePatternMutation, updateRecurringClassScheduleMutation } from '@/services/client/@tanstack/react-query.gen';
                ...
                <FormField
                    ...
                    render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                                <FormControl>
                                    <Input type='number' placeholder="e.g 2 to mean every 2 Days or 2 Weeks or 2 Months" {...field} />
                                </FormControl>
                            </FormControl>
                            <FormMessage />
                        </FormItem>
    ...
    const handleSubmit = async (values: ScheduleFormValues) => {
        ...
        if (scheduleId) {
            ...
            createClassSchedule.mutate(
                { path: { uuid: classId as string }, query: { startDate: payload.start_date, endDate: payload.end_date } },
                {
                    onSuccess: (data: any) => {
                        qc.invalidateQueries({
                            queryKey: getClassRecurrencePatternQueryKey({ path: { uuid: "" } }),
...
function ClassDialog({ isOpen, setOpen, editingClassId, initialValues, onSuccess, onCancel }: ClassDialogProps) {
    return (
        <Dialog open={isOpen} onOpenChange={setOpen}>
            <DialogContent className='flex max-w-6xl flex-col p-0'>
                <DialogHeader className='border-b px-6 py-4'>
                    <DialogTitle className='text-xl'>{editingClassId ? "Edit Class" : "Add Class"}</DialogTitle>
...
function RecurrenceDialog({ isOpen, setOpen, onSuccess, editingRecurrenceId, initialValues, onCancel }: RecurrenceDialogProps) {
    return (
        <Dialog open={isOpen} onOpenChange={setOpen}>
            <DialogContent className='flex max-w-6xl flex-col p-0'>
                <DialogHeader className='border-b px-6 py-4'>
                    <DialogTitle className='text-xl'>{editingRecurrenceId ? "Edit Recurrence" : "Add Recurrence"}</DialogTitle>
                    <DialogDescription className='text-muted-foreground text-sm'>
...
function ScheduleDialog({ isOpen, setOpen, onSuccess, editingScheduleId, editingClassId, initialValues, onCancel }: ScheduleDialogProps) {
    return (
        <Dialog open={isOpen} onOpenChange={setOpen}>
            <DialogContent className='flex max-w-6xl flex-col p-0'>
                <DialogHeader className='border-b px-6 py-4'>
                    <DialogTitle className='text-xl'>{editingScheduleId ? "Edit Recurring Class Schedule" : "Create Recurring Class Schedule"}</DialogTitle>
                    <DialogDescription className='text-muted-foreground text-sm'>
                        {editingScheduleId ? "Edit class recurring schedule" : "Create a new class recurring schedule"}
                    </DialogDescription>
                </DialogHeader>
