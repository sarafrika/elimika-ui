...
export const ComponentBlock = ({
  ...
  return (
    <div className='space-y-4 rounded border border-gray-300 p-2'>
      {/* Component Name */}
      <Input placeholder='Assessment Component Name' {...register(`components.${index}.name`)} />
                  ...
                  <select
                    ...
                    onChange={e => {
                      ...
                      if (selectedLevel) {
                        ...
                        setValue(
                          `components.${index}.grading.${gradingIndex}.uuid`,
                          selectedLevel.uuid
                        );
                        setValue(
                          `components.${index}.grading.${gradingIndex}.grading_level_uuid`,
                          selectedLevel.uuid
                        );
                      } else {
      ...
      <Button
        type='button'
        variant='outline'
        onClick={() => append({ name: '', description: '', points: 0, uuid: '' })}
      >
...
export function AddRubricForm({
  ...
  const createRubricIsPending =
    createRubric.isPending || createRubricComponent.isPending || createRubricScores.isPending;
  const updateRubricIsPending =
    updateRubric.isPending || updateRubricComponent.isPending || updateRubricScores.isPending;
                    ...
                    updateRubricScores.mutate(
                      {
                        path: {
                          rubricUuid: rubricId,
                          criteriaUuid: criterionId as string,
                          scoringUuid: grading?.scoring_uuid as string,
                        },
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className={`space-y-8 ${className}`}>
        ...
        <div className='flex justify-end gap-2 pt-6'>
          ...
          >
            {(createRubricIsPending || updateRubricIsPending) && <Spinner />}
