  ...
  return (
    <div className='space-y-6'>
      ...
      <Alert>
        ...
        <AlertDescription>
          Please ensure all required compliance documents are provided. This helps maintain training
          quality and ensures legal compliance for educational activities.
        </AlertDescription>
      </Alert>
            ...
            <div key={declaration.id} className='flex items-start space-x-3'>
              <Checkbox
                ...
                onCheckedChange={checked =>
                  handleComplianceAgreement(declaration.id, checked as boolean)
                }
                className='mt-1'
              />
              <Label htmlFor={declaration.id} className='cursor-pointer text-sm leading-relaxed'>
                {declaration.text}
              </Label>
            </div>
          ))}
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-muted-foreground text-sm'>
                {
                  COMPLIANCE_AREAS.filter(area => getComplianceStatus(area.id) === 'completed')
                    .length
                }{' '}
                of {COMPLIANCE_AREAS.filter(area => area.required).length} required areas completed
              </p>
            </div>
            <div className='text-right'>
              <div className='flex items-center gap-2'>
                {COMPLIANCE_AREAS.filter(area => area.required).every(
                  ...
                  <>
                    ...
                    <span className='font-medium text-yellow-600'>Pending Documents</span>
                  </>
                )}
              </div>
            </div>
