...
import { deleteAssignmentMutation, getAllAssignmentsOptions, getAllAssignmentsQueryKey } from "@/services/client/@tanstack/react-query.gen";
...
export default function AssignmentsPage() {
  ...
  const { data, isLoading, isFetched } = useQuery(getAllAssignmentsOptions({ query: { pageable: {} } }))
  ...
  const confirmDelete = () => {
    ...
    deleteMutation.mutate({ path: { uuid: editingAssignmetId } }, {
      onSuccess: () => {
    ...
    <div className='space-y-6'>
      ...
      {isFetched && assignments?.length === 0 &&
        <div className='text-muted-foreground rounded-lg border border-dashed p-12 text-center'>
          ...
          <p className='text-muted-foreground mt-2'>You can create assignments for your lesson to get started.</p>
        </div>
      }

      {!isLoading && isFetched && <div className='space-y-3'>
