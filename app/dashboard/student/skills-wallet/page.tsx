    ...
    return (
        <div className="flex flex-col items-end">
            ...
            <div className="mt-1 flex items-center gap-2">
                <div className="bg-background rounded-md border px-3 py-1.5 font-mono text-sm">{WALLET_ID}</div>
...
function LegendRow({ colorClass, label, value }: { colorClass: string; label: string; value: string | number }) {
    return (
...
// ---------- Mock data ----------
const ICON_MAP: Record<string, LucideIcon> = { Code2, BarChart3, Rocket, Mic, Palette, Cloud, Megaphone, Globe, BookOpen };
...
const MOCK_SKILLS: SkillRecord[] = [
    { id: "s1", name: "React", level: "Expert", proficiency_pct: 95, category: "Technology", last_used: "2026-07-28", icon_key: "Code2", tint: "bg-primary/10 text-primary" },
    { id: "s2", name: "Data Analysis", level: "Advanced", proficiency_pct: 82, category: "Business", last_used: "2026-07-20", icon_key: "BarChart3", tint: "bg-secondary text-secondary-foreground" },
    { id: "s3", name: "Public Speaking", level: "Advanced", proficiency_pct: 78, category: "Soft Skills", last_used: "2026-06-30", icon_key: "Mic", tint: "bg-warning/10 text-warning" },
    { id: "s4", name: "UI/UX Design", level: "Intermediate", proficiency_pct: 64, category: "Design", last_used: "2026-07-10", icon_key: "Palette", tint: "bg-muted text-muted-foreground" },
    { id: "s5", name: "Cloud Infrastructure", level: "Intermediate", proficiency_pct: 58, category: "Cloud Computing", last_used: "2026-05-15", icon_key: "Cloud", tint: "bg-primary/10 text-primary" },
    { id: "s6", name: "Project Delivery", level: "Expert", proficiency_pct: 90, category: "Engineering", last_used: "2026-07-31", icon_key: "Rocket", tint: "bg-success/10 text-success" },
    { id: "s7", name: "Digital Marketing", level: "Beginner", proficiency_pct: 30, category: "Business", last_used: "2026-04-02", icon_key: "Megaphone", tint: "bg-warning/10 text-warning" },
    { id: "s8", name: "Technical Writing", level: "Intermediate", proficiency_pct: 55, category: "Soft Skills", last_used: "2026-06-18", icon_key: "BookOpen", tint: "bg-success/10 text-success" },
];

const MOCK_PORTFOLIO: PortfolioRecord[] = [
...
const MOCK_COMPETENCIES: CompetencyRecord[] = [
...
const MOCK_EXPERIENCES: ExperienceRecord[] = [
...
const MOCK_ACHIEVEMENTS: AchievementRecord[] = [
    { id: "a1", name: "Fast Learner", description: "Completed 5 courses in a single month.", points: 250, achieved_at: "2026-07-15", status: "Completed", color_key: "bg-primary", progress: null },
    { id: "a2", name: "Top Contributor", description: "Ranked in the top 10 on the leaderboard.", points: 400, achieved_at: "2026-06-28", status: "Completed", color_key: "bg-secondary", progress: null },
    { id: "a3", name: "Certification Streak", description: "Earned 3 credentials back to back.", points: 300, achieved_at: "2026-07-01", status: "Completed", color_key: "bg-success", progress: null },
    { id: "a4", name: "Mentor in Training", description: "Guide 10 peers through course material.", points: 150, achieved_at: null, status: "In Progress", color_key: "bg-warning", progress: 60 },
];

const MOCK_VERIFICATION_EVENTS: VerificationEventRecord[] = [
...
function OverviewTab() {
    ...
    const stats = [
        { icon: BookOpen, label: "Total Skills", value: skillsRows.length, tint: "bg-primary/10 text-primary" },
        { icon: Target, label: "Competencies", value: (compsQ.data ?? []).length, tint: "bg-secondary text-secondary-foreground" },
        {
            icon: Award,
            label: "Certificates",
            value: (credsQ.data ?? []).filter((c) => c.status === "Verified").length,
            tint: "bg-warning/10 text-warning",
        },
        { icon: Briefcase, label: "Projects", value: (portfolioQ.data ?? []).length, tint: "bg-primary/10 text-primary" },
        {
            icon: Trophy,
            label: "Achievements",
            value: (achQ.data ?? []).filter((a) => a.status === "Completed").length,
            tint: "bg-success/10 text-success",
        },
    ];
    const topSkills = skillsRows.slice(0, 5).map((s) => ({
        name: s.name,
        level: s.level,
        pct: s.proficiency_pct,
        icon: ICON_MAP[s.icon_key] ?? Sparkles,
        tint: s.tint ?? "bg-muted text-muted-foreground",
    }));
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                {stats.map((s) => (
                    <StatCard key={s.label} {...s} sub={<span className="text-primary">View details →</span>} />
                ))}
            </div>
                ...
                <Card>
                    ...
                    <CardContent className="flex items-center gap-6">
                        ...
                        <div className="flex-1 space-y-2">
                            <LegendRow colorClass="bg-primary" label="Expert" value="20%" />
                            <LegendRow colorClass="bg-secondary" label="Advanced" value="35%" />
                            <LegendRow colorClass="bg-warning" label="Intermediate" value="25%" />
                            <LegendRow colorClass="bg-success" label="Beginner" value="20%" />
                        </div>
                    </CardContent>
                </Card>
...
function SkillsTab() {
    ...
    const count = (lvl: string) => rows.filter((s) => s.level === lvl).length;
    const pct = (n: number) => `${Math.round((n / total) * 100)}% of total`;
    const stats = [
        { icon: BookOpen, label: "Total Skills", value: rows.length, tint: "bg-primary/10 text-primary" },
        {
            icon: Star,
            label: "Expert",
            value: count("Expert"),
    ...
    const skills = rows.map((s) => ({
        ...
        icon: ICON_MAP[s.icon_key] ?? Sparkles,
        tint: s.tint ?? "bg-muted text-foreground",
    }));
    const catMap = new Map<string, number>();
    rows.forEach((s) => catMap.set(s.category, (catMap.get(s.category) ?? 0) + 1));
    const catColors: Record<string, string> = {
        Technology: "bg-primary",
        Engineering: "bg-success",
        Design: "bg-secondary",
        "Soft Skills": "bg-warning",
        Business: "bg-primary",
        "Cloud Computing": "bg-primary",
    };
    const categories = Array.from(catMap.entries()).map(([name, c]) => ({
        name,
        count: c,
        colorClass: catColors[name] ?? "bg-muted",
    }));
    const recommended = [
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Explore and manage the skills you've acquired and are developing.</p>
                </div>
                <div className="flex items-center gap-3">
                ...
                <Card className="lg:col-span-3">
                        ...
                        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                            <div className="flex gap-4 text-sm">
                                {["All Skills", "By Category", "In Progress", "Completed", "Bookmarked"].map((t, i) => (
                                    <button
                                        key={t}
                                        className={`pb-1 ${i === 0 ? "border-b-2 border-primary text-primary font-medium" : "text-muted-foreground"}`}
                                    >
                                        {t}
                                    </button>
                                ))}
                    ...
                    <CardContent className="space-y-3">
                        {skills.map((s) => (
                            <div
                                key={s.name}
                                className="flex items-center gap-4 rounded-lg border p-3 hover:border-primary/40 transition"
                            >
                                ...
                                <div className="min-w-0 flex-1 grid grid-cols-1 md:grid-cols-4 gap-2 md:gap-4 items-center">
                                    ...
                                    <div>
                                        <p className="text-[11px] text-muted-foreground">Proficiency</p>
                                        <div className="flex items-center gap-2 mt-1">
                        ...
                        <CardContent className="space-y-3">
                            {categories.map((c) => (
                                <div key={c.name}>
                                    <div className="flex items-center justify-between text-sm">
                                        <span>{c.name}</span>
                                        <span className="text-muted-foreground">{c.count} skills</span>
                                    </div>
                        ...
                        <CardContent className="space-y-3">
                            {recommended.map((r) => (
                                <div key={r.name} className="flex items-start gap-3">
                                    <div className="h-8 w-8 rounded-md bg-warning/10 text-warning grid place-items-center">
...
function PortfolioTab() {
    ...
    const rows = q.data ?? [];
    const totalViews = rows.reduce((a: number, r) => a + (r.views ?? 0), 0);
    const totalLikes = rows.reduce((a: number, r) => a + (r.likes ?? 0), 0);
    const stats = [
        ...
        { icon: Star, label: "Avg. Rating", value: "4.7", sub: "★★★★★", tint: "bg-warning/10 text-warning" },
    ];
        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Showcase your best work and track your impact.</p>
                </div>
                <div className="flex items-center gap-3">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {items.map((p) => (
                    <Card key={p.title} className="overflow-hidden">
                        <div className="bg-background/80 relative h-32 bg-gradient-to-br from-background to-muted grid place-items-center">
                            {p.featured && <Badge className="absolute top-2 left-2 bg-primary">Featured</Badge>}
...
function CredentialsTab() {
    ...
    const orgs = new Set(rows.map((c) => c.org)).size;
    const stats = [
        ...
        { icon: Trophy, label: "Issuing Orgs", value: orgs, tint: "bg-secondary text-secondary-foreground" },
    ];
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {creds.map((c) => (
                    <Card key={c.id} className="overflow-hidden">
                        <div className="relative h-28 bg-gradient-to-br from-primary/10 to-success/10 grid place-items-center">
                        ...
                        <CardContent className="p-4">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">Issued: {c.issued}</p>
                            <div className="mt-2">
...
function AlertBadge(props: React.ComponentProps<typeof Clock>) {
    return <Clock {...props} />;
}
...
function CompetenciesTab() {
    ...
    const mastered = data.filter((r) => r.pct >= 80).length;
    const inProgress = data.filter((r) => r.pct > 0 && r.pct < 80).length;
    const notStarted = data.filter((r) => r.pct === 0).length;
    const overall = data.length ? Math.round(data.reduce((s: number, r) => s + (r.pct ?? 0), 0) / data.length) : 0;
    const stats = [
        {
            icon: CheckCircle2,
            label: "Mastered",
        ...
        pct >= 80
            ...
            : pct >= 40
                ? {
                    ...
                    grad: "from-warning to-warning/70",
                    bg: "bg-warning/5",
                    text: "text-warning",
                    bar: "bg-warning/50",
                }
                : {
                    ...
                    ring: "ring-border",
                    grad: "from-muted-foreground to-muted-foreground/80",
                    bg: "bg-muted/50",
                ...
                <Card>
                    <CardContent className="p-4 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-muted-foreground mt-1">avg. across all badges</p>
                        </div>
                    </CardContent>
                </Card>
                {stats.map((s) => (
                                    ...
                                    <CardContent className="p-5">
                                        <div className="flex items-start gap-4">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-start justify-between gap-2">
                                                    <p className="font-semibold text-foreground truncate">{r.competency}</p>
                                        ...
                                        <div className="mt-4">
                                            <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                                                <span>Progress</span>
                                                <span className="tabular-nums font-medium text-foreground">{r.pct}%</span>
                                            </div>
...
function ExperienceTab() {
    ...
    const data = q.data ?? [];
    const catCount = (c: string) => data.filter((r) => r.category === c).length;
    const stats = [
        {
            icon: Briefcase,
                                    ...
                                    <div className="flex items-start justify-between gap-4">
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {it.current && <Badge className="bg-success/10 text-success border-0">Current</Badge>}
                                            </div>
...
function AchievementsTab() {
    ...
    const totalPts = completed.reduce((a: number, r) => a + (r.points ?? 0), 0);
    const stats = [
        {
            icon: Trophy,
    ...
    const badges = data.map((a) => ({
        ...
        colorClass: a.color_key ?? "bg-muted-foreground",
        status: a.status,
                    ...
                    <Card key={b.name} className="overflow-hidden">
                        <CardContent className="p-4">
                            <div className="flex items-start justify-between">
                                <div className={`h-14 w-14 ${b.colorClass} text-primary-foreground rounded-xl grid place-items-center`}>
...
// ---------- Verification ----------
const SOURCE_META: Record<string, { icon: LucideIcon; title: string; desc: string; tint: string }> = {
    assessment: {
        icon: FileCheck2,
        title: "Assessments",
