                ...
                {item.hasSubmitted ? (
                  <div className='flex gap-2'>
                    ...
                    {item?.submission?.status === 'returned' && (
                      <Button asChild>
                        <Link
                          href={`/dashboard/student/assignment/${item?.assignment?.uuid}`}
                  ...
                  <Button asChild>
                    <Link
                      href={`/dashboard/student/assignment/${item?.assignment_uuid}`}
                      className='flex items-center gap-2'
                    >
                      <FileQuestion className='h-4 w-4' />
                      Attempt Assignment
                ...
                {item.quiz_uuid && (
                  <Button>
                    <Link
                      ...
                      href={`/dashboard/student/assignment/quiz/${item?.quiz_uuid}`}
                    >
                      <FileQuestion className='h-4 w-4' />
                      Attempt quiz
                    </Link>
        ...
        <SheetContent side='right' className='w-full overflow-y-auto p-2 sm:max-w-3xl sm:p-4'>
          <div className='space-y-6'>
            <div>
              ...
              <div className='border-destructive/30 bg-destructive/10 text-destructive mt-4 flex items-start gap-2 rounded-md border px-3 py-2 text-sm'>
                <span className='text-destructive mt-0.5'>⚠</span>
                <p>
                  You are currently in Assignment <span className='font-medium'>preview mode</span>.{' '}
                  <a
                    href={`/dashboard/student/assignment/${previewAssignment?.uuid}`}
                    className='text-destructive shrink-0 font-medium underline underline-offset-2 hover:opacity-80'
                  >
        ...
        <SheetContent side='right' className='w-full overflow-y-auto p-2 sm:max-w-4xl sm:p-6'>
          <div className='space-y-6'>
            <div className='border-b pb-4'>
              ...
              {/* 🚨 Preview warning */}
              <div className='border-destructive/30 bg-destructive/10 text-destructive mt-4 flex items-start justify-between gap-3 rounded-md border px-3 py-2 text-sm'>
                <div className='flex items-start gap-2'>
                  <span className='text-destructive mt-0.5'>⚠</span>
                  <p>
                    You are currently in Quiz <span className='font-medium'>preview mode</span>.{' '}
                    <a
                      href={`/dashboard/student/assignment/quiz/${previewQuiz?.uuid}`}
                      className='text-destructive shrink-0 font-medium underline underline-offset-2 hover:opacity-80'
                    >
                      Go to quiz page
                    </a>{' '}
        ...
        <div className='max-w-full space-y-3 p-3'>
            ...
            <>
              ...
              {selectedStudentSubmissions.length > 0 ? (
                ...
                <div className='text-muted-foreground rounded-md border border-dashed p-5 text-sm'>
                  {selectedStudent
                    ? 'No assignment submissions are available for the selected lesson yet.'
          ...
          {activePanel === 'notes' ? (
            <div className='space-y-3'>
              ...
              {sentNotes.length === 0 ? (
                <div className='text-muted-foreground rounded-md border border-dashed p-5 text-sm'>
                  Session notes for this class will appear here.
                </div>
              ) : null}
            </div>
          ) : null}
...
export default function StudentClassTrainingPage({
  classId: classIdProp,
  ...
  const contentInitialLoading =
    (isLoading || rosterLoading || lessonsLoading) && lessonModules.length === 0;
    ...
    <main className='text-foreground fixed inset-0 z-50 flex flex-col overflow-hidden bg-[color-mix(in_oklch,var(--el-brand-50)_80%,var(--background))]'>
      <header className='border-border/70 bg-primary text-primary-foreground flex h-16 shrink-0 items-center justify-between gap-3 border-b px-3 shadow-sm sm:px-4'>
        <div className='flex min-w-0 items-center gap-3'>
          ...
          >
            <Link href='/dashboard/student/learning-hub' aria-label='Back to classes'>
      ...
      <section className='grid min-h-0 flex-1 gap-0 overflow-hidden xl:grid-cols-[minmax(0,1fr)_420px] 2xl:grid-cols-[minmax(0,1fr)_460px]'>
