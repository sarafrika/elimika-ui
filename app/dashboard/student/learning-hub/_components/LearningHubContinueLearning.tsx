              ...
              return (
                ...
                >
                  <div className='space-y-1'>
                    <div className='h-[150px] overflow-hidden rounded-sm border'>
                      {classItem?.bannerUrl ? (
                        <Image
                          src={
                            toAuthenticatedMediaUrl(classItem?.bannerUrl) || classItem?.bannerUrl
                          }
                          alt='Course banner'
                    ...
                    <div className='flex items-center justify-between gap-2 text-[0.7rem]'>
                      {isCompleted ? (
                        <span className='bg-success/15 text-success inline-flex items-center gap-1 rounded-full px-2 py-1 font-medium'>
                          <CheckCircle2 className='size-3.5' />
                          Class Completed
                        </span>
                      ) : (
                        <span className='text-muted-foreground'>{classItem.statusLabel}</span>
                      )}
                      ...
                      <div className='flex items-center gap-1.5'>
                        {[0, 1, 2, 3].map(index => (
                          <span
                            ...
                            className={cn(
                              'size-2 rounded-full',
                              index < Math.max(1, Math.round(classItem.progress / 25))
                                ? 'bg-primary'
                                : 'bg-muted'
                            )}
                          />
                    ...
                    {isCompleted && (
                      <div className='bg-background/60 rounded-md px-3 py-2'>
                    ...
                    {isCompleted ? (
                      <div className='grid w-full grid-cols-2 gap-2'>
                        <Link
                          ...
                          className={cn(
                            'text-primary-foreground inline-flex w-full items-center justify-center rounded-[8px] px-3 py-2 text-center text-[0.78rem] font-medium transition hover:opacity-95',
                      ...
                      <>
                        <Link
                          ...
                          href={`/dashboard/student/learning-hub/classes/${classItem.id}`}
                          className={cn(
                            'bg-primary text-primary-foreground inline-flex flex-1 items-center justify-center rounded-[8px] px-3 py-2 text-[0.78rem] font-medium transition hover:opacity-95',
                            buttonAccentClasses[classItem.accent]
                          )}
                        >
                          {classItem.ctaLabel}
