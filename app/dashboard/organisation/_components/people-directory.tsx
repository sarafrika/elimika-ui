...
const initialMemberFormState = (defaultRole: DomainNameEnum): MemberFormState => ({
  firstName: '',
  middleName: '',
  lastName: '',
  email: '',
  ...
  return [
    {
      ...
      cell: ({ row }) => (
        <Link
          href={row.original.uuid ? `/dashboard/organisation/people/${row.original.uuid}` : '#'}
...
export interface PeopleDirectoryProps {
  /** When set, the directory is scoped to a single organisation role. */
  domain?: DomainNameEnum;
  title: string;
  description: string;
  /** Label for the add-member button. */
  addLabel?: string;
}

/**
 * Reusable members directory. Rendered on its own dedicated route per user type
 * (Students, Instructors) and as the all-members "People" view. Keeps all real
 * data fetching + the create-member flow; only the scope/title change per page.
 */
export function PeopleDirectory({ domain, title, description, addLabel = 'Add member' }: PeopleDirectoryProps) {
  const organisation = useOrganisation();
  const qc = useQueryClient();
  const organisationUuid = organisation?.uuid ?? '';

  const defaultRole: DomainNameEnum =
    domain && validOrganisationRoles.has(domain) ? domain : 'organisation_user';

  const [isMemberSheetOpen, setIsMemberSheetOpen] = useState(false);
  const [memberForm, setMemberForm] = useState<MemberFormState>(() =>
    initialMemberFormState(defaultRole)
  );

  const usersQuery = useQuery({
    ...(domain
      ? getUsersByOrganisationAndDomainOptions({
          path: { uuid: organisationUuid, domainName: domain },
        })
      : getUsersByOrganisationOptions({
  ...
  const totalItems = getTotalFromMetadata(usersPage.metadata) || users.length;
  ...
  const branchesWithUuid = useMemo(
    () => branches.filter((branch): branch is TrainingBranch & { uuid: string } => Boolean(branch.uuid)),
    [branches]
  );
  ...
  const roleCounts = useMemo(
    () => ({
      orgUsers: users.filter(u => getOrganisationRole(u, organisationUuid) === 'organisation_user').length,
      instructors: users.filter(u => getOrganisationRole(u, organisationUuid) === 'instructor').length,
      students: users.filter(u => getOrganisationRole(u, organisationUuid) === 'student').length,
    }),
    [users, organisationUuid]
  );
  ...
  return (
    <OrgPage>
      ...
      <div className='mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4 2xl:gap-5'>
        {usersQuery.isLoading ? (
          ...
          <>
            <KpiCard title='Total Members' value={totalItems} icon={<Users className='h-5 w-5' />} variant='primary' />
            <KpiCard title='Org Users' value={roleCounts.orgUsers} icon={<Building2 className='h-5 w-5' />} variant='indigo' />
            <KpiCard title='Instructors' value={roleCounts.instructors} icon={<BookOpen className='h-5 w-5' />} variant='amber' />
            <KpiCard title='Students' value={roleCounts.students} icon={<GraduationCap className='h-5 w-5' />} variant='green' />
          </>
        )}
      </div>
      ...
      <section className={elimikaDesignSystem.spacing.content}>
        {usersQuery.isLoading ? (
          ...
          <div className={elimikaDesignSystem.components.emptyState.container}>
            ...
            <h3 className={elimikaDesignSystem.components.emptyState.title}>No {title.toLowerCase()} found</h3>
