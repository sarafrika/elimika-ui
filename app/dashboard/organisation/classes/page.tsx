...
export default function ClassesPage() {
  ...
  const refresh = () =>
    queryClient.invalidateQueries({
      queryKey: getClassDefinitionsForOrganisationOptions({ path: { organisationUuid } }).queryKey,
    });
  ...
  const suspendSelected = () => {
    ...
    if (active.length)
      toast.success(`Suspending ${active.length} class${active.length === 1 ? '' : 'es'}`);
    setSelectedIds(new Set());
  };

  const inViewSelected = filtered.filter(r => selectedIds.has(r.uuid)).length;
  const allViewChecked =
    filtered.length > 0 && inViewSelected === filtered.length
      ? true
      : inViewSelected > 0
        ? 'indeterminate'
        : false;
  ...
  const toggleOne = (uuid: string, v: boolean) =>
    setSelectedIds(prev => {
      const next = new Set(prev);
      if (v) next.add(uuid);
          ...
          {selectedIds.size > 0 && (
            <div className='bg-muted/40 flex flex-wrap items-center justify-between gap-2 rounded-lg border px-3 py-2'>
              <span className='text-sm'>
                <span className='font-medium'>{inViewSelected}</span> of {filtered.length} selected
              </span>
              <div className='flex items-center gap-2'>
                ...
                >
                  <PauseCircle className='mr-2 h-4 w-4' /> Suspend selected
                </Button>
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className='w-10'>
                    <Checkbox
                      aria-label='Select all classes in view'
                    ...
                    >
                      <TableCell>
                        <Checkbox
                          ...
                          checked={selectedIds.has(r.uuid)}
                          onCheckedChange={v => toggleOne(r.uuid, Boolean(v))}
                        />
                      </TableCell>
                      ...
                      <TableCell className='text-right whitespace-nowrap'>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align='end'>
                            <DropdownMenuItem
                              onClick={() => router.push('/dashboard/organisation/calendar')}
                            >
                              <Eye className='mr-2 h-4 w-4' /> View schedule
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() => router.push('/dashboard/organisation/students')}
                            >
                            ...
                            {r.status === 'Active' && (
                              <>
                                ...
                                <DropdownMenuItem
                                  ...
                                  disabled={deactivateMutation.isPending}
                                  onClick={() => suspend(r.uuid)}
                                >
                                  <PauseCircle className='mr-2 h-4 w-4' /> Suspend class
                                </DropdownMenuItem>
                              </>
                            )}
                          </DropdownMenuContent>
