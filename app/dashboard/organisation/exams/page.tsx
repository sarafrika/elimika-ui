  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-6 px-3 py-4 sm:px-5 lg:px-6 2xl:max-w-[1840px]">
      <PageHeader
        ...
        action={
          <Button onClick={() => toast.info('Schedule exam', { description: 'Exams are built as quizzes within a class lesson.' })}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search exams or courses" value={query} onChange={e => setQuery(e.target.value)} className="pl-9" />
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 p-12 text-center">
              ...
              <div className="font-medium">{orgQuizzes.length === 0 ? 'No exams yet' : 'No exams match'}</div>
              <p className="text-sm text-muted-foreground">
                {orgQuizzes.length === 0 ? 'Exams built in your class lessons will appear here.' : 'Try adjusting your filters.'}
              </p>
            </div>
          ) : (
                    ...
                    return (
                      <TableRow key={q.uuid}>
                        <TableCell className="whitespace-nowrap">
                          ...
                          {q.description && <div className="max-w-xs truncate text-xs text-muted-foreground">{q.description}</div>}
                        </TableCell>
                        <TableCell className="whitespace-nowrap">{classTitleByUuid.get(q.class_definition_uuid ?? '') ?? '—'}</TableCell>
                        <TableCell className="whitespace-nowrap">
                          {q.is_timed ? (
                        ...
                        <TableCell className="whitespace-nowrap">{q.attempts_allowed ?? '—'}</TableCell>
                        <TableCell className="whitespace-nowrap">{q.passing_score != null ? `${q.passing_score}%` : '—'}</TableCell>
                        <TableCell className="whitespace-nowrap">
