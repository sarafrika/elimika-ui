      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card className="border-l-4 border-l-primary">
          <CardContent className="p-6">
            ...
            <div className="text-xs text-muted-foreground">On the waiting list</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-l-4 border-l-success">
          <CardContent className="p-6">
            <div className="text-2xl font-bold">
              {capacity != null ? Math.max(0, capacity - extractList<Enrollment>(enrollmentsQuery.data).filter(e => (e.status ?? '').toUpperCase() === 'ENROLLED').length) : '—'}
            </div>
          ...
          {loading ? (
            ...
            <div className="p-6">
              <EmptyState
                ...
                description={activeClassDef ? `${activeClassDef.title} has no waitlisted students.` : 'Select a class to view its waiting list.'}
              />
            </div>
          ) : (
              ...
              <Table className="min-w-[640px]">
                ...
                <TableBody>
                  {waitlist.map((e, idx) => {
                    ...
                    return (
                      <TableRow key={e.uuid}>
                        <TableCell className="whitespace-nowrap text-sm text-muted-foreground">{idx + 1}</TableCell>
                        ...
                        <TableCell className="whitespace-nowrap text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                onClick={() => toast.success('Offer sent', { description: `${fullName(student)} has been offered a seat.` })}
                              >
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() => toast.error('Removed from waitlist', { description: fullName(student) })}
                              >
