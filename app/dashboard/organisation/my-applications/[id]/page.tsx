...
export default function ApplicationDetailPage() {
  ...
  const classDefs: ClassDefinition[] = useMemo(
    () =>
      (classesQuery.data?.data ?? [])
        ...
        .filter(cd => cd?.course_uuid === app?.course_uuid),
  ...
  const instructorNames = useMemo(() => {
    const byUuid = new Map(
      extractPage<User>(instructorsQuery.data).items.map(u => [
        u.uuid,
  ...
  const plannedStartRaw =
    classDefs
      .map(cd => cd.default_start_time)
      .filter(Boolean)
      .sort()[0] ?? null;
  const plannedStart = plannedStartRaw ? dayjs(plannedStartRaw).format('DD MMM YYYY') : null;

  const modes = useMemo(
    () => RATE_TIERS.filter(t => Number(app?.rate_card?.[t.key] ?? 0) > 0).map(t => t.method),
    [app]
  );
  ...
  const timeline = [
    { icon: FileText, label: 'Application submitted', when: submitted, done: true },
    {
      icon: UserCheck,
      label: 'Under review',
      when: isDecided || !isPending ? 'Reviewed' : 'In progress',
      done: isDecided || !isPending,
    },
    {
      icon: CalendarCheck,
      label: 'Decision recorded',
      when: decided ?? 'Pending',
      done: isDecided,
    },
  ];
    ...
    return (
      <div className='mx-auto max-w-md space-y-4 py-16 text-center'>
        ...
        <p className='text-muted-foreground text-sm'>
          This application reference doesn't exist or has been archived.
        </p>
        <Button asChild>
          <Link href='/dashboard/organisation/my-applications'>Back to applications</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className='mx-auto w-full max-w-[1400px] space-y-6 px-3 py-4 sm:px-5 lg:px-6'>
      ...
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <Button
          variant='ghost'
          size='sm'
          onClick={() => router.push('/dashboard/organisation/my-applications')}
        >
        ...
        <div className='relative h-56 w-full sm:h-72 lg:h-80'>
          ...
          <div className='absolute inset-x-0 bottom-0 flex flex-col gap-4 p-5 sm:p-8'>
            <div className='flex flex-wrap items-center gap-2'>
              <Badge className={cn('text-white', statusTone(app.status))}>
                {pretty(app.status)}
              </Badge>
              {cats[0] && (
                <Badge
            ...
            <div className='max-w-3xl'>
              ...
              <p className='mt-2 max-w-2xl text-sm text-white/85 sm:text-base'>
                Application to train · reference{' '}
                <span className='font-mono tracking-tight'>{ref}</span>
              </p>
            </div>
            <div className='flex flex-wrap items-center gap-4 text-sm text-white/90'>
              {reviewer && (
                <>
                  <div className='flex items-center gap-2'>
                    <div className='flex h-9 w-9 items-center justify-center rounded-full border-2 border-white/70 bg-white/15 text-xs font-semibold text-white uppercase'>
                      {reviewer.slice(0, 2).toUpperCase()}
                    </div>
                    <div>
                      <div className='leading-tight font-medium'>{reviewer}</div>
              ...
              <div className='flex items-center gap-1.5'>
                ...
                {cohort > 0 ? `Cohort of ${cohort}` : pretty(app.applicant_type)}
              </div>
            </div>
          </div>
        </div>
          ...
          <Tabs value={tab} onValueChange={setTab}>
            ...
            <TabsContent value='overview' className='mt-6 space-y-6'>
              <Card>
                <CardHeader>
                  <CardTitle className='flex items-center gap-2 text-base'>
                    <Sparkles className='h-4 w-4 text-teal-600' /> Application summary
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <dl className='grid grid-cols-1 gap-x-8 gap-y-5 sm:grid-cols-2'>
                    ...
                    <DetailItem
                      ...
                      value={pretty(app.applicant_type)}
                    />
                    <DetailItem
                    ...
                    <DetailItem
                      ...
                      label='Decision date'
                      value={decided ?? 'Pending'}
                    />
                    <DetailItem
                      icon={<Users className='h-4 w-4' />}
                      label='Cohort size'
                      value={cohort > 0 ? String(cohort) : 'Not set'}
                    />
                    <DetailItem
                      icon={<MapPin className='h-4 w-4' />}
                      label='Venue'
                      value={branch ?? 'Not set'}
                    />
                    <DetailItem
                      icon={<CalendarCheck className='h-4 w-4' />}
                      label='Planned start'
                      value={plannedStart ?? 'Not scheduled'}
                    />
                    <DetailItem
                      icon={<UserCheck className='h-4 w-4' />}
                      label='Instructor'
                      value={instructorNames.length ? instructorNames.join(', ') : 'Unassigned'}
                    />
                  </dl>
                </CardContent>
              </Card>
                  ...
                  <ul className='grid gap-2 sm:grid-cols-2'>
                    {checklist.map(c => (
                      ...
                      >
                        ...
                        <span className={cn('min-w-0 truncate', !c.ok && 'text-muted-foreground')}>
                          {c.label}
                        </span>
                      </li>
                    ))}
              ...
              {applicantNotes && (
                <Card>
                  <CardHeader>
                    <CardTitle className='text-base'>Your submission notes</CardTitle>
                  </CardHeader>
                  <CardContent>
            ...
            <TabsContent value='timeline' className='mt-6'>
              <Card>
                ...
                <CardContent>
                  <ol className='border-muted relative space-y-6 border-l-2 pl-6'>
                    {timeline.map(s => (
                      <li key={s.label} className='relative'>
                        <span
                          className={cn(
                            'absolute -left-[33px] flex h-8 w-8 items-center justify-center rounded-full border-2',
                            s.done
            ...
            <TabsContent value='notes' className='mt-6'>
              <Card>
                <CardHeader>
                  <CardTitle className='flex items-center gap-2 text-base'>
                    <MessageSquare className='h-4 w-4 text-teal-600' /> Reviewer notes
                  </CardTitle>
                </CardHeader>
                <CardContent className='space-y-4'>
                  {notes ? (
                    <div className='rounded-lg border-l-4 border-teal-500 bg-teal-50/60 p-4'>
                      ...
                      <p className='text-foreground/80 text-sm leading-relaxed'>{notes}</p>
                    </div>
                  ) : (
                    <p className='text-muted-foreground text-sm'>No reviewer notes yet.</p>
                  )}
                  <p className='text-muted-foreground flex items-center gap-1 text-xs'>
                    <Lock className='h-3.5 w-3.5' /> Notes are part of the immutable audit record.
                  </p>
                </CardContent>
              </Card>
            </TabsContent>
                ...
                <CardContent>
                  <div className='grid gap-3 sm:grid-cols-2'>
                    ...
                    <ActionButton
                      ...
                      onClick={() => router.push('/dashboard/organisation/courses/catalog')}
                    />
                    <ActionButton
                    ...
                    {isPending && (
                      <ActionButton
                        ...
                        description='Cancel this pending request'
        ...
        <aside className='space-y-6 lg:sticky lg:top-4 lg:self-start'>
          <Card className='overflow-hidden'>
            <div className='bg-gradient-to-br from-teal-600 to-teal-800 p-5 text-white'>
              <div className='text-xs tracking-wide text-white/70 uppercase'>Status</div>
            ...
            <CardContent className='space-y-4 pt-5'>
              <div className='space-y-2'>
                <div className='flex items-center justify-between text-xs'>
                  <span className='text-muted-foreground'>Cohort readiness</span>
              ...
              <dl className='space-y-3 text-sm'>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <dd className='font-medium'>{cohort > 0 ? cohort : '—'}</dd>
                </div>
                <div className='flex items-center justify-between'>
                  <dt className='text-muted-foreground'>Venue</dt>
                  <dd className='max-w-[55%] truncate text-right font-medium'>{branch ?? '—'}</dd>
                </div>
                <div className='flex items-center justify-between'>
                  <dt className='text-muted-foreground'>Planned start</dt>
                  <dd className='font-medium'>{plannedStart ?? '—'}</dd>
                </div>
                <div className='flex items-center justify-between'>
                ...
                <div className='flex items-center justify-between'>
                  ...
                  <dd className='font-medium'>{reviewer ?? 'Unassigned'}</dd>
                </div>
              </dl>
            </CardContent>
          </Card>
...
function ActionButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'hover:bg-muted/50 flex items-start gap-3 rounded-lg border p-4 text-left transition',
