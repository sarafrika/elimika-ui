...
interface DashboardClientLayoutProps extends DashboardChildrenTypes {
  initialProfile: any;
  session: Session;
}

function DashboardContent(dashboardProps: Omit<DashboardClientLayoutProps, 'initialProfile' | 'session'>) {
  ...
  const userDashboards = Object.keys(dashboardProps).reduce(
    (a: { [key: string]: ReactNode }, b: string) =>
      orgAdminDomains.includes(b as OrgDomainType) ? { ...a, [b]: dashboardProps[b] } : a,
    {}
  );
  const currentDashboard = activeDomain ? userDashboards[activeDomain] : dashboardProps.children;
...
export default function DashboardClientLayout(props: DashboardClientLayoutProps) {
  const { initialProfile, session, ...dashboardProps } = props;

  return (
    <UserProfileProvider initialData={initialProfile}>
