...
export function AudioPlayer({ isOpen, onClose, audioUrl, title, description }: AudioPlayerProps) {
    ...
    return (
      <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/95'>
        <div className='max-w-md space-y-4 p-8 text-center'>
          ...
          <div className='bg-muted/10 text-muted-foreground/70 rounded-md p-3 text-xs break-all'>
            {audioUrl}
          </div>
          <Button onClick={onClose} variant='outline'>
            Close Player
  ...
  return (
    <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/95'>
      <div
        ref={containerRef}
        className='border-border bg-card w-full max-w-2xl rounded-xl border p-6 shadow-xl sm:p-8'
      >
        <div className='space-y-6'>
          ...
          {audioSource === 'direct' && (
            <div className='flex items-center justify-center'>
              <div
                className='from-primary/20 to-primary/10 rounded-full bg-gradient-to-br p-8 sm:p-12'
                onMouseEnter={() => setShowControls(true)}
                onMouseMove={() => setShowControls(true)}
                onMouseLeave={() => setShowControls(false)}
              >
                <div className='bg-primary/10 flex h-24 w-24 items-center justify-center rounded-full sm:h-32 sm:w-32'>
                  {isPlaying ? (
                    <Pause className='text-primary h-10 w-10 sm:h-14 sm:w-14' />
                  ) : (
                    <Play className='text-primary h-10 w-10 sm:h-14 sm:w-14' />
                  )}
                </div>
              </div>
            </div>
          ...
          {audioSource === 'direct' && (
            <div className='space-y-4'>
              ...
              <div className='flex items-center gap-2 sm:gap-3'>
                <span className='text-muted-foreground font-mono text-xs whitespace-nowrap sm:text-sm'>
                  {formatTime(currentTime)}
                </span>
                <div
                  className='bg-muted relative h-2 flex-1 cursor-pointer rounded-full transition-all hover:h-3 sm:h-2'
              ...
              <div className='flex items-center justify-center gap-2 sm:gap-4'>
                ...
                <div className='flex items-center gap-2 sm:gap-3'>
                  <Button
                    size='sm'
                    variant='ghost'
                    className='text-muted-foreground hover:text-foreground h-8 w-8 p-0 sm:h-10 sm:w-10'
                    onClick={toggleMute}
                  >
                    {isMuted || volume === 0 ? (
                  ...
                  <input
                    ...
                    className='accent-primary h-1 w-20 cursor-pointer sm:w-24'
                  />
                </div>
