...
export const BookingSummary: React.FC<Props> = ({
    ...
    const totalFee = selectedSlots.reduce(
        (sum, slot) => sum + calculateFee(slot.duration),
        0
    );
    ...
    return (
        <Card className="p-6 space-y-6 sticky top-6">
            ...
            <div>
                ...
                <p className="text-sm text-muted-foreground">{instructor.professional_headline}</p>
            </div>
            ...
            <div className="text-xs text-muted-foreground bg-muted p-3 rounded-lg">
                <p>
                    <strong>Note:</strong> Your booking will be confirmed once the payment is
                    processed. You will receive a confirmation email with all the details.
                </p>
            </div>
        </Card>
    );
