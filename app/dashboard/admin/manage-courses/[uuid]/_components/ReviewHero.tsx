...
export function ReviewHero({ course, creatorName }: { course: Course; creatorName?: string }) {
  const banner = course.banner_url
    ? toAuthenticatedMediaUrl(course.banner_url) || course.banner_url
    : undefined;
  ...
  return (
    <header className='border-border/70 bg-card relative overflow-hidden rounded-md border shadow-sm'>
      {/* Banner backdrop */}
      <div className='from-primary/15 via-muted/60 to-muted/30 relative h-40 w-full bg-gradient-to-r sm:h-48'>
        {banner ? (
      ...
      <div className='relative -mt-12 flex flex-col gap-4 px-5 pb-5 sm:flex-row sm:items-end'>
        <div className='border-border/70 bg-muted/40 flex h-28 w-40 shrink-0 items-center justify-center overflow-hidden rounded-md border shadow-md'>
          {thumb ? (
            // eslint-disable-next-line @next/next/no-img-element
