...
export function QuizViewer({ quiz, open, onOpenChange }: QuizViewerProps) {
  ...
  }, [quiz?.uuid, quizquestions?.data?.length, queryStates.allLoaded, queryStates.anyError]);
                ...
                <Card key={question.uuid} className='border-2'>
                  ...
                  <CardContent className='space-y-2'>
                    ...
                    {question.options && question.options.length > 0 ? (
                      question.options.map((option: any, optIndex: number) => (
                        ...
                        >
                          ...
                          <div className='flex-1'>
                            <p className='text-foreground text-sm'>{option.text}</p>
                            {option.isCorrect && (
                              <Badge
                                variant='outline'
                      ...
                      /* Essay / Short Answer */
                      <div className='border-muted bg-muted/30 rounded-lg border-2 p-3'>
                        <p className='text-muted-foreground text-sm italic'>
                          {question.type === 'ESSAY'
                            ? 'Essay question - written response required'
