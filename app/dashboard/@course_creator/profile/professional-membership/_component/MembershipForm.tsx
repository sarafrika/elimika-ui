...
const professionalMembershipSchema = z.object({
  professional_bodies: z.array(CourseCreatorMembershipSchema),
});

type CourseCreatorMembershipType = z.infer<typeof CourseCreatorMembershipSchema>;
...
export default function ProfessionalBodySettings() {
  ...
  const passMember = (mem: CourseCreatorProfessionalMembership) => ({
    ...mem,
  ...
  const addMemMutation = useMutation(addCourseCreatorMembershipMutation());
  const updateMemMutation = useMutation(updateCourseCreatorMembershipMutation());
