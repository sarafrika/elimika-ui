...
import {
  useClassesEnrollmentsByIds,
  useInstructorClasses,
  useInstructorRatingSummaries,
...
export default function ApprovedInstructors() {
  ...
  const instructorUuids = instructors?.map(instructor => instructor.uuid) ?? [];
  ...
  const classUuids = useMemo(
    () =>
      Object.values(instructorClassesMap ?? {}).flatMap(classes =>
        // @ts-ignore
      ...
      instructors
        ...
        .map(instructor => {
          ...
          return {
            ...
            ratingSummary: instructorRatingSummariesMap?.[instructor.uuid] ?? {
          ...
          <Table className='min-w-[1000px]'>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className='flex flex-col gap-0.5 text-start'>
                  <p>Split Ratio (%)</p>
                  <p className='text-xs'>Creator/Instructor</p>
                </TableHead>
            ...
            <TableBody>
              {filtered.map(instructor => (
                <TableRow
                  ...
                  onClick={() =>
                    router.push(
                      `/dashboard/course-creator/instructors/${encodeURIComponent(instructor.uuid)}`
                    )
                  }
                >
                  {/* Instructor */}
                  ...
                  <TableCell className='align-top'>
                    <div className='space-y-3'>
                      {instructor.courses.map(course => (
                        <div key={course.uuid} className='min-w-[180px]'>
                  ...
                  <TableCell className='align-top'>
                    <div className='space-y-3'>
                      {instructor.courses.map(course => {
                        ...
                        return (
                          <div key={course.uuid} className='text-muted-foreground h-5 text-sm'>
                    ...
                    <div className='space-y-3'>
                      {instructor.courses.map(course => {
                        ...
                        courseClasses.forEach(classItem => {
                          ...
                          const enrollments = classEnrollmentsMap.get(classUuid) ?? [];
                  ...
                  <TableCell className='text-muted-foreground align-top'>
                    {instructor.ratingSummary?.review_count > 0
                      ...
                      } ${instructor.ratingSummary.review_count === 1 ? 'review' : 'reviews'})`
                      : 'No reviews'}
                  </TableCell>
