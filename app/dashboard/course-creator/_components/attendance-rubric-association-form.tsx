...
export type AttendanceRubricAssociationFormProps = {
  courseUuid: string;
  associatedBy: string;
};

const USAGE_CONTEXT = 'attendance_assessment';

export const AttendanceRubricAssociationForm = ({
  courseUuid,
  associatedBy,
}: AttendanceRubricAssociationFormProps) => {
  ...
  // Get the existing associated rubric for attendance
    ...
    associateRubricMut.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          toast.success(
            existingRubric
              ? 'Attendance rubric updated successfully!'
              : 'Attendance rubric associated successfully!'
          );
        },
        onError: err => {
          toast.error(err instanceof Error ? err.message : 'Failed to save attendance rubric');
        },
      }
    );
  };
  ...
  return (
    <div className='bg-card rounded-xl border p-6 shadow-sm'>
      <div className='mb-6 flex items-center justify-between border-b pb-4'>
        <div>
          <h3 className='text-foreground text-lg font-bold'>Attendance Rubric</h3>
          <p className='text-muted-foreground mt-1 text-sm'>
            Associate a rubric for attendance assessments
          </p>
        </div>
      </div>
      {/* 
        ...
        <div className='flex flex-col gap-6'>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <p className='text-muted-foreground text-xs'>
              This rubric will be used for attendance assessments
            </p>
          </div>
          ...
          <div className='flex flex-col gap-2'>
            ...
            <p className='text-muted-foreground text-xs'>
              Choose a rubric to use for attendance assessments
            </p>

            <Select value={selectedRubricUuid} onValueChange={setSelectedRubricUuid}>
          ...
          <div className='flex flex-col gap-2'>
            <div className='flex items-center gap-3'>
              <Switch checked={isPrimaryRubric} onCheckedChange={setIsPrimaryRubric} />
              <Label className='text-foreground text-sm font-medium'>Set as Primary Rubric</Label>
            </div>
            <p className='text-muted-foreground text-xs'>
          ...
          {selectedRubricDetails && (
            <>
              ...
              <div className='bg-muted/50 rounded-lg border p-4'>
                <h4 className='text-foreground mb-2 text-sm font-semibold'>Rubric Details</h4>
                <ul className='text-muted-foreground space-y-1 text-sm'>
                  <li>
                    <strong>Rubric:</strong> {selectedRubricDetails.title}
                  </li>
                  <li>
                    <strong>Usage:</strong> Attendance Assessment
                  </li>
                  <li>
