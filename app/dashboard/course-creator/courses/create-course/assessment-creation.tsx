...
import {
    addQuestionOptionMutation, addQuizQuestionMutation,
...
import { AssignmentCreationForm } from '../../_components/assignment-creation-form';
import { QuizCreationForm, QuizPayload } from '../../_components/quiz-creation-form';
...
// combined listing card that opens this form. `selectedLessonId`/`selectedLesson` are lifted
// to the parent for the same reason (the combined listing is scoped per lesson too).
type AssessmentCreationFormProps = {
    ...
    // Pass a uuid to open the form pre-loaded in edit mode; leave undefined/null for "create new".
    initialQuizUuid?: string | null;
    initialAssignmentUuid?: string | null;
    // Let the parent refresh its combined listing / close the sheet after a successful mutation.
    onQuizSaved?: (quizUuid: string) => void;
    onQuizDeleted?: () => void;
    onAssignmentSaved?: (assignmentUuid: string) => void;
...
const AssessmentCreation = ({
    ...
    setSelectedLessonId,
    setSelectedLesson,
    initialQuizUuid = null,
    initialAssignmentUuid = null,
    onQuizSaved,
    onQuizDeleted,
    onAssignmentSaved,
    onAssignmentDeleted,
    ...
    const [activeQuizUuid, setActiveQuizUuid] = useState<string | null>(initialQuizUuid);
    ...
    const [assignmentId, setAssignmentId] = useState<string | null>(initialAssignmentUuid);
    ...
    const handleDeleteQuiz = useCallback(
        ...
        [deleteQuiz, qc, selectedLessonId, onQuizDeleted]
    );
    ...
    function parseQuizText(text: string) {
        ...
        return blocks.map(parseQuestionBlock);
    }
    ...
    const handleBulkImport = async () => {
        try {
            ...
            const parsedQuestions = parseQuizText(bulkText);

            setQuizData(prev => ({
                ...prev,
                [selectedLessonId]: parsedQuestions,
            }));
    ...
    const createAssignmentForLesson = useCallback(
                ...
                createAssignmentMut.mutate(
                    ...
                    },
                    {
                        onSuccess: (data: MessageWithData<Assignment>) => {
                            ...
                            if (resolvedUuid) {
                                onAssignmentSaved?.(resolvedUuid);
        ...
        [createAssignmentMut, qc, selectedLessonId, onAssignmentSaved]
    );
    ...
    const updateAssignmentForLesson = useCallback(
                ...
                updateAssignmentMut.mutate(
                    ...
                    },
                    {
                        onSuccess: () => {
                            ...
                            onAssignmentSaved?.(assignmentUuid);
                            resolve();
                        },
                        onError: error => {
                            reject(error);
        ...
        [updateAssignmentMut, qc, selectedLessonId, onAssignmentSaved]
    );
    ...
    const deleteAssignmentForLesson = useCallback(
        ...
        [deleteAssignmentMut, qc, selectedLessonId, onAssignmentDeleted]
    );
                        ...
                        <Sheet open={bulkSheetOpen} onOpenChange={setBulkSheetOpen}>
                            <SheetContent className='overflow-y-auto sm:max-w-2xl'>
                                <SheetHeader>
                                    ...
                                    <SheetDescription>
                                        Paste quiz questions using the bulk upload template.
                                    </SheetDescription>
                                </SheetHeader>
