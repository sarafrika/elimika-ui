...
export const buildTemplatePayload = (
  ...
  template_css:
    'body{font-family:Arial,sans-serif;background:#f8fafc;padding:24px}.card{border:2px solid #0f172a;padding:32px;text-align:center;background:white;}',
});
...
const AwardCertificatesPage = () => {
  ...
  const studentCertificates = useMemo<Certificate[]>(() => {
    ...
    return certificates.filter(
      cert => cert.course_uuid === courseUuid || cert.program_uuid === classData?.program_uuid
    );
  ...
  const attendanceSummary = useMemo(() => {
    const filteredEnrollments = selectedEnrollmentId
      ? rosterAllEnrollments.filter(entry => entry?.enrollment?.student_uuid === selectedStudentId)
  ...
  }, [rosterAllEnrollments, selectedEnrollmentId, activeInstanceStudents]);
  ...
  const assessmentRows = useMemo(() => {
    return enrichedScheduledAssignmentSubmission
      .filter(schedule => schedule.submission?.enrollment_uuid === enrollmentUuid)
      .map(schedule => ({
  ...
  const handleAwardCertificate = async () => {
    ...
    try {
      ...
      if (createdCertificate?.certificate_number) {
        toast.success(
          `Certificate issued to ${selectedStudent?.user?.full_name ?? 'the learner'}.`
        );
      } else {
  ...
  const handleVerifyCertificate = async () => {
    ...
    try {
      ...
      toast.success(
        isValid ? 'Certificate verified successfully.' : 'Certificate could not be verified.'
      );
    } catch (error) {
  ...
  if (isLoading || rosterLoading) {
    return (
      <div className='text-muted-foreground flex min-h-[480px] items-center justify-center rounded-xl border border-dashed p-8 text-sm'>
  ...
  if (isError) {
    return (
      <div className='border-destructive/20 bg-destructive/5 text-destructive rounded-xl border p-6 text-sm'>
  ...
  return (
    <div className='flex h-full min-h-0 flex-col gap-4 p-4'>
      <div className='border-border/70 bg-card/90 flex flex-col gap-3 rounded-xl border p-4 shadow-sm'>
        <div className='flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between'>
          <div>
            <p className='text-muted-foreground text-xs font-semibold tracking-[0.2em] uppercase'>
              Certificate awards
            </p>
            <h1 className='text-foreground text-2xl font-semibold'>{className}</h1>
            <p className='text-muted-foreground mt-1 text-sm'>
              Review learner performance and issue a certificate once a class has concluded.
            </p>
          </div>
      ...
      <div className='grid min-h-0 flex-1 gap-4 lg:grid-cols-[320px_minmax(0,1fr)]'>
        <aside className='border-border/70 bg-card/90 flex min-h-0 flex-col overflow-hidden rounded-xl border shadow-sm'>
          <div className='border-border/70 border-b p-3'>
            <div className='flex items-center justify-between gap-3'>
              <div>
                <p className='text-foreground text-sm font-semibold'>Enrolled learners</p>
                <p className='text-muted-foreground text-xs'>
                  {activeInstanceStudents.length} students enrolled in this class / course
                </p>
              </div>
              ...
              {filteredRoster.map(entry => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-lg border p-3 text-left transition-colors ${isSelected ? 'border-primary/40 bg-primary/8' : 'hover:bg-muted/60 border-transparent'}`}
                  >
                    <div className='flex items-start gap-2.5'>
                      <Avatar className='border-border/60 size-9 border'>
                        <AvatarImage
                          src={toAuthenticatedMediaUrl(entry.user?.profile_image_url) || ''}
                          alt={entry.user?.full_name ?? 'Student'}
                        />
                        <AvatarFallback>
                          {(entry.user?.full_name ?? 'ST').slice(0, 2).toUpperCase()}
                        </AvatarFallback>
                      </Avatar>
        ...
        <main className='flex min-h-0 flex-col gap-4 overflow-hidden'>
          {!selectedStudent ? (
            <div className='bg-card/70 text-muted-foreground flex min-h-[320px] items-center justify-center rounded-xl border border-dashed p-8 text-center text-sm'>
              Select a learner from the roster to view their progress and award a certificate.
            </div>
          ) : (
            <>
              <div className='border-border/70 bg-card/90 rounded-xl border p-4 shadow-sm'>
                <div className='mb-4 flex flex-wrap items-start justify-between gap-3'>
                  <div>
                    ...
                    <p className='text-muted-foreground mt-1 text-sm'>
                      {courseName} ·{' '}
                      {dayjs(activeSchedule?.end_time ?? new Date()).format('MMM D, YYYY')}
                    </p>
                  </div>
                  ...
                  <div className='flex flex-wrap gap-2'>
                    ...
                    <Button
                      ...
                      disabled={
                        !activeCertificate?.certificate_number || createCertificate.isPending
                      }
                    >
                ...
                <div className='grid gap-3 md:grid-cols-3'>
                  <div className='border-border/60 bg-background/80 rounded-lg border p-3'>
                    ...
                    <p className='text-foreground mt-2 text-2xl font-semibold'>
                      {attendanceSummary.presentCount}/{attendanceSummary.totalCount}
                    </p>
                    <p className='text-muted-foreground text-sm'>present out of this session</p>
                  </div>
                  <div className='border-border/60 bg-background/80 rounded-lg border p-3'>
                    <p className='text-muted-foreground text-xs tracking-[0.16em] uppercase'>
                      Current grade
                    ...
                    <p className='text-muted-foreground text-sm'>Weighted course grade</p>
                  </div>
                  <div className='border-border/60 bg-background/80 rounded-lg border p-3'>
                    <p className='text-muted-foreground text-xs tracking-[0.16em] uppercase'>
                      Certificate state
                    </p>
                    <p className='text-foreground mt-2 flex items-center gap-2 text-lg font-semibold'>
                      {activeCertificate?.uuid ? (
                        ...
                        <>
                          <GraduationCap className='text-warning h-4 w-4' />
                          Pending
                        </>
                      )}
                    </p>
              ...
              <div className='flex min-h-0 flex-col gap-4'>
                <div className='border-border/70 bg-card/90 flex min-h-0 flex-col gap-4 overflow-hidden rounded-xl border p-4 shadow-sm'>
                  <div className='flex items-center justify-between'>
                    <div>
                      <p className='text-foreground text-sm font-semibold'>Performance overview</p>
                      <p className='text-muted-foreground text-sm'>
                        Attendance, graded tasks and weighted course results.
                      </p>
                    </div>
                  ...
                  <ScrollArea className='min-h-0 flex-1'>
                    <div className='space-y-3'>
                      <div className='text-md font-bold'>
                        Assessments (Tasks, Exams and Projects)
                      </div>
                      ...
                      {assessmentRows.length > 0 ? (
                        assessmentRows.map((item, index) => (
                          <div
                            ...
                            className='border-border/60 bg-background/80 rounded-lg border p-3'
                          >
                ...
                <div className='border-border/70 bg-card/90 flex min-h-0 flex-col gap-4 overflow-hidden rounded-xl border p-4 shadow-sm'>
                  <div>
                    <p className='text-foreground text-sm font-semibold'>Certificate preview</p>
                    <p className='text-muted-foreground text-sm'>
                      Use the current PDF certificate template as a live preview before issuance.
                    </p>
                  </div>

                  {certificatePreviewData ? (
                    <div className='border-border/60 bg-background/80 min-h-0 flex-1 overflow-hidden rounded-lg border p-2'>
                      <CertificatePage certData={certificatePreviewData} />
                    </div>
                  ) : (
                    <div className='text-muted-foreground flex min-h-[280px] items-center justify-center rounded-lg border border-dashed p-6 text-center text-sm'>
