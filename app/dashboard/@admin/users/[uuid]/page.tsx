...
import { fetchUserCredentials } from '@/services/admin/credential-review';
...
import { CredentialReviewSection } from '../_components/CredentialReviewSection';
import { UserIdentityForm } from '../_components/UserIdentityForm';

function domainList(user: User): string[] {
  const raw = user.user_domain;
  const domains = Array.isArray(raw) ? raw : raw ? [raw] : [];
  return domains.map(domain => String(domain));
}
...
/** Credentials stream independently — this async child is wrapped in its own <Suspense>. */
async function CredentialsSection({ userUuid, verifier }: { userUuid: string; verifier: string }) {
  const documents = await fetchUserCredentials(userUuid);
  return <CredentialReviewSection documents={documents} verifierIdentity={verifier} />;
}

export default async function UserDetailPage({ params }: { params: Promise<{ uuid: string }> }) {
  const { uuid } = await params;
  const [{ data }, session] = await Promise.all([
    getUserByUuid({ path: { uuid } }).catch(() => ({ data: undefined })),
    auth(),
  ]);
    ...
    <main className={adminTheme.page}>
      <div className={adminTheme.pageStack}>
        <div>
          ...
          {/* Identity header — renders instantly, no skeleton */}
        ...
        <div className='grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]'>
          ...
          <SectionCard
            ...
            description='Preview uploaded documents and verify them in place.'
          >
            <Suspense fallback={<SectionCardSkeleton rows={3} withHeader={false} className='border-none shadow-none' />}>
              <CredentialsSection userUuid={uuid} verifier={verifier} />
            </Suspense>
          </SectionCard>
        </div>
      </div>
