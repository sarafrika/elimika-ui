...
import { getUserByUuidOptions, searchCourseCreatorsOptions, searchInstructorsOptions, searchStudentsOptions } from '../../services/client/@tanstack/react-query.gen';
...
import { SharedUserProfile, TabDefinition, UserDomain } from '../dashboard/_profile-components/types';
...
function DomainBadge({ domain, profile }: { domain: UserDomain; profile: SharedUserProfile }) {
    if (domain === 'instructor' && profile.admin_verified) {
        return (
            <Badge variant="outline" className="border-primary/60 bg-primary/10 text-xs font-semibold tracking-wide uppercase">
                ✓ Verified Instructor
            </Badge>
        );
    }
...
export default function PublicUserProfilePage() {
    ...
    const user = { ...userData?.data, student, instructor, courseCreator: course_creator, course_creator };
