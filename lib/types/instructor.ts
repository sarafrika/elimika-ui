...
export const TrainingExperienceSchema = z.object({
  ...
  start_date: z.preprocess(val => (typeof val === 'string' ? new Date(val) : val), z.date()),
  end_date: z
    .preprocess(val => {
...
export const ProfessionalBodySchema = z.object({
  ...
  member_since: z.preprocess(val => (typeof val === 'string' ? new Date(val) : val), z.date()),
  end_year: z
    .preprocess(val => {
      if (val === '' || val === null) return null;
      if (typeof val === 'string') return new Date(val);
...
export const InstructorFormSchema = z.object({
  ...
  website: z
    .union([z.string().url({ message: 'Please enter a valid URL.' }), z.string().nullish()])
    .optional()
    .nullable()
