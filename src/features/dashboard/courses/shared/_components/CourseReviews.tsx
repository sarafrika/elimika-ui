  ...
  if (reviews.length === 0) {
    return (
      <div className='text-muted-foreground py-10 text-center'>
        <h2 className='text-foreground text-lg font-semibold'>Student Reviews</h2>
        <p className='mt-2 text-sm'>There are no reviews for this course yet.</p>
      </div>
    );
  }
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          <h2 className='text-foreground text-lg font-semibold'>Student Reviews</h2>
          <p className='text-muted-foreground text-sm'>{reviews.length} reviews in total</p>
        </div>

        <div className='text-right'>
          <p className='text-primary text-2xl font-bold'>{averageRating.toFixed(1)}</p>
      ...
      <div className='space-y-4'>
        {filteredReviews.length > 0 ? (
          filteredReviews.map(review => {
            ...
            return (
              <div key={review.uuid} className='border-border bg-background rounded-xl border p-5'>
                {/* Header */}
                <div className='flex items-start justify-between gap-4'>
                  <div>
                    <h3 className='text-foreground text-sm font-semibold'>{review.headline}</h3>
