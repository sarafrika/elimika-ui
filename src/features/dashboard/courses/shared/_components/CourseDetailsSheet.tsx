...
export function CourseDetailsSheet({
  ...
  const { isLoading: lessonsLoading, lessons: lessonsWithContent } = useCourseLessonsWithContent({
    courseUuid: isCourse ? resolvedId : '',
  });
  ...
  const avgRating =
    reviewCount > 0 ? reviews.reduce((sum, r) => sum + (r.rating || 0), 0) / reviewCount : null;
  ...
  const studentRequirements = (
    isCourse ? (courseReqResp?.data?.content ?? []) : aggregatedRequirements
  ).filter(
        ...
        {isLoading || !title ? (
          ...
          <div className='space-y-5 text-sm'>
            ...
            <div className='flex flex-wrap gap-1.5'>
              ...
              {isCourse && course?.level && <Badge variant='outline'>{course.level}</Badge>}
            ...
            <div className='bg-muted/60 text-foreground grid grid-cols-2 gap-3 rounded-lg border p-3 text-xs'>
              {avgRating != null && (
              ...
              >
                <ol className='space-y-2'>
                  {courseLessons.map((l: Lesson, i: number) => (
                    <li key={l.uuid ?? i} className='bg-card rounded-md border p-2'>
              ...
              >
                <ol className='space-y-2'>
                  {bundledCourses.map((c, i) => (
                    <li key={c.uuid ?? i} className='bg-card rounded-md border p-2'>
                      <div className='text-foreground text-sm font-medium'>
                        {i + 1}. {c.name}
                      </div>
                      {c.description && (
...
function Section({
  ...
  return (
    <div>
      <div className='text-primary mb-1.5 inline-flex items-center gap-1.5 text-xs font-semibold tracking-wide uppercase'>
        {icon}
        {title}
      </div>
