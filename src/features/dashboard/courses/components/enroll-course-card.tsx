...
function StarRating({ rating, count }: { rating: number; count: number }) {
  return (
    <div className='flex items-center gap-1'>
      <span className='text-foreground text-[0.78rem] font-semibold'>{rating.toFixed(1)}</span>
        ...
        return (
          <span
            ...
            className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[0.68rem] font-medium ${
              tagVariantStyles[variant] ?? 'border-border bg-muted text-foreground'
            }`}
          >
            {tag}
          </span>
...
function MetaStrip({
  ...
  return (
    <div className='border-border/60 flex flex-wrap items-start gap-x-3 gap-y-2 border-t pt-3'>
      {items.map(({ icon: Icon, label, sub }) => (
        <div key={sub} className='flex min-w-0 items-center gap-1'>
          <Icon className='text-primary/70 size-3.5 shrink-0' />
          <div className='leading-tight'>
            <p className='text-foreground text-[0.7rem] font-semibold'>{label}</p>
            <p className='text-muted-foreground text-[0.65rem]'>{sub}</p>
          </div>
        </div>
      ))}
    </div>
...
function CourseInfoRow({
  ...
  return (
    <div className='border-border flex flex-wrap items-center justify-start gap-x-4 gap-y-1.5 border-t pt-2.5'>
      {/* Instructor */}
      <div className='flex min-w-0 flex-1 items-center gap-1.5'>
        <div className='bg-primary/10 text-primary flex size-5 shrink-0 items-center justify-center overflow-hidden rounded-full text-[0.6rem] font-semibold'>
          {instructorInitial || <GraduationCap className='text-muted-foreground size-3' />}
        </div>
        <div className='min-w-0 leading-tight'>
          <p className='text-muted-foreground text-[0.6rem]'>Instructor</p>
          <p className='text-foreground truncate text-[0.7rem] font-medium'>{instructorName}</p>
        </div>
      </div>
      ...
      {startDate && (
        <div className='flex min-w-0 flex-1 items-center gap-1'>
          ...
          <div className='leading-tight'>
            <p className='text-muted-foreground text-[0.6rem]'>Starts</p>
            <p className='text-foreground text-[0.7rem] font-medium'>{startDate}</p>
          </div>
        </div>
      )}
      ...
      <div className='flex min-w-0 flex-1 items-center gap-1'>
        ...
        <div className='leading-tight'>
          <p className='text-muted-foreground text-[0.6rem]'>Enrolled</p>
          <p className='text-foreground text-[0.7rem] font-medium'>
      ...
      <article className='border-border/70 bg-card flex w-full min-w-0 flex-col gap-2.5 rounded-[12px] border p-2 shadow-sm transition-shadow hover:shadow-md'>
        ...
        <div className='border-border bg-muted relative min-h-[200px] overflow-hidden rounded-[8px] border sm:h-[140px]'>
          ...
          <Button
            ...
            asChild
            className='bg-foreground/80 text-background hover:bg-foreground absolute top-2 left-2 h-6 rounded-full px-1.5 text-[0.58rem] font-medium shadow-none backdrop-blur-sm transition-colors'
          >
            <Link href={href}>
          ...
          {/* Full badge */}
          {isFull && (
            <Badge className='bg-destructive text-destructive-foreground absolute top-2 right-2 rounded-full shadow-none'>
              Full
            </Badge>
          )}
          ...
          {!instructorView && variant === 'full' && cls?.catalogue?.variant_code && (
            <Button
              ...
              size='sm'
              variant='secondary'
              className='bg-background/80 hover:bg-background absolute right-2 bottom-2 rounded-full px-2 shadow-none backdrop-blur-sm'
              onClick={e => {
                e.stopPropagation();
        ...
        <div className='space-y-1.5 px-0.5'>
          <h3 className='text-foreground line-clamp-2 text-[1rem] leading-snug font-semibold'>
            {cls?.title}
          </h3>
        ...
        <div className='flex items-center justify-between px-0.5'>
          ...
          <div className='flex items-center gap-2'>
            <Button
              variant='secondary'
              size='sm'
              className='bg-muted/50 hover:bg-muted h-8 gap-1 shadow-none'
            ...
            <Button
              variant='secondary'
              size='sm'
              className='bg-muted/50 hover:bg-destructive-50 hover:text-destructive dark:hover:bg-destructive-950/20 h-8 gap-1 shadow-none'
            >
        ...
        {/* ── Description ── */}
        <div className='text-muted-foreground line-clamp-3 px-0.5 text-[0.74rem] leading-relaxed'>
        ...
        {variant === 'full' && maxParticipants > 0 && (
          <div className='space-y-1 px-0.5'>
            <div className='text-muted-foreground flex items-center justify-between text-[0.68rem]'>
        ...
        <div className='flex flex-wrap items-center justify-between gap-2 px-0.5 py-1'>
          ...
          {instructorView ? (
            ...
            <button
              ...
              disabled={isEnrolled}
              className={`inline-flex items-center justify-center gap-1.5 rounded-[7px] px-3.5 py-1.5 text-[0.78rem] font-medium transition hover:opacity-90 disabled:cursor-not-allowed ${
                isEnrolled
