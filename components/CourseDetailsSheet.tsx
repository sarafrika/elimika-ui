...
function CourseSheet({
  ...
  const averageRating = reviewCount
    ? (reviewItems.reduce((sum, review) => sum + (review.rating || 0), 0) / reviewCount).toFixed(1)
    : null;
  ...
  if (!course) {
    return (
      <div className='border-border text-muted-foreground mt-6 rounded-2xl border border-dashed p-8 text-center text-sm'>
  ...
  return (
    <div className='mt-4 space-y-5 text-sm'>
      {courseImage ? (
        <img src={courseImage} alt={course.name} className='h-48 w-full rounded-2xl object-cover' />
      ) : (
          ...
          <div className='space-y-3'>
            {courseNames.length === 0 ? (
              ...
              courseNames.map((courseName, index) => (
                <div key={`${courseName}-${index}`} className='border-border rounded-xl border p-3'>
        ...
        <CardContent className='space-y-4 p-4'>
          ...
          <div className='grid gap-3 sm:grid-cols-2'>
            ...
            <StatCard
              icon={CalendarDays}
              label='Requirements'
              value={aggregatedRequirements.length}
            />
          </div>
