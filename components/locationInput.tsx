...
const Geocoder = dynamic(
    () => import("@mapbox/search-js-react").then(mod => mod.Geocoder) as Promise<ForwardRefExoticComponent<GeocoderProps & RefAttributes<unknown>>>,
    { ssr: false }
);
...
export default function LocationInput({ ...props }: Omit<GeocoderProps, "accessToken">) {
    return (
        <Geocoder {...props} accessToken={mapboxToken} options={{
            language: "en"
        }} />
    )
}
