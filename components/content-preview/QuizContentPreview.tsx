...
function QuestionOptionsPreview({
    ...
    return (
      <div className='space-y-2'>
        {options.map((option, index: number) => {
          ...
          const selected = typeof answer === 'string' && answer === optionUuid;
      ...
      <div className='rounded-xl border p-4'>
        {isInstructor ? (
          <div className='space-y-3'>
            <p className='text-sm font-semibold'>Model Answer</p>
    ...
    return (
      <div className='space-y-2'>
        {options.map((option, index: number) => (
          ...
          >
            <div className='flex-1'>
              <p className='text-sm font-medium'>{option.left_text}</p>
            </div>
            ...
            <div className='flex-1'>
              {isInstructor ? (
                <div className='text-right text-sm'>{option.right_text}</div>
              ) : (
                <select
                  disabled={submitted}
                ...
                >
                  ...
                  {options.map((rightOption: QuizQuestionOptionPreview) => {
                    const rightOptionUuid = rightOption.uuid ?? rightOption.option_text;

                    return (
                      <option key={rightOptionUuid} value={rightOptionUuid}>
  ...
  return <div className='text-muted-foreground text-sm'>No options available.</div>;
}
...
export function QuizContentPreview({
  ...
  const [isQuestionsOpen, setIsQuestionsOpen] = useState(questionsOpen);
  ...
  const handleSubmitQuiz = async () => {
    try {
      const formattedAnswers = Object.entries(answers).map(([questionUuid, value]) => ({
        question_uuid: questionUuid,

        selected_option_uuid: typeof value === 'string' && value.length < 100 ? value : undefined,

        text_answer: typeof value === 'string' ? value : undefined,

        matching_pairs: typeof value === 'object' ? value : undefined,
      }));
        ...
        <CardHeader className='space-y-5'>
          <div className='flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between'>
            ...
            <Badge
              ...
              className={cn('w-fit', quiz.active && 'bg-green-600 hover:bg-green-700')}
            >
          ...
          <div className='flex flex-wrap items-center gap-2'>
            <Badge variant='secondary'>
              ...
              {quiz.time_limit_minutes || 0} mins
            </Badge>

            <Badge variant='outline'>
              <Target className='mr-1 h-3.5 w-3.5' />
              Pass Score: {quiz.passing_score || 0}%
            </Badge>
      ...
      <div className='space-y-4'>
        ...
        >
          ...
          <div className='flex items-center gap-1'>
            <span className='text-sm font-medium'>{isQuestionsOpen ? 'Collapse' : 'Expand'}</span>
            ...
            {questions.length === 0 ? (
              ...
              <>
                {questions.map((question, index: number) => {
                  ...
                  return (
                    <Card key={questionUuid} className='overflow-hidden shadow-none'>
                      <CardContent className='space-y-5 pt-6'>
                        ...
                        <QuestionOptionsPreview
                          ...
                          role={role}
                          answer={answers[questionUuid]}
                          submitted={submitted}
                          onChange={value => updateAnswer(questionUuid, value)}
                        />
                      </CardContent>
                    </Card>
                  );
                ...
                {role === 'preview' && (
                  <div className='flex justify-end pt-4'>
                    <Button disabled={submitted} onClick={handleSubmitQuiz}>
