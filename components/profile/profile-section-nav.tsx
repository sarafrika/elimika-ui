...
export function ProfileSectionNav({ items, className, ...props }: ProfileSectionNavProps) {
  ...
  return (
    <div className={cn('mx-auto w-full max-w-5xl px-0 sm:px-6 lg:px-8', className)} {...props}>
      <div className='border-border/60 bg-background/80 supports-[backdrop-filter]:bg-background/60 relative overflow-hidden rounded-full border shadow-sm ring-1 ring-black/5 backdrop-blur'>
          ...
          {items.map(item => {
            ...
            return (
              <Link
                ...
                href={item.href}
                className={cn(
                  'inline-flex items-center justify-center rounded-full px-4 py-1.5 text-sm font-medium whitespace-nowrap transition-colors',
                  isActive
