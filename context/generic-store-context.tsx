...
export default function StoreContextProvider({ storeName, children, store }: StoreProviderProps) {
  const Store = useStore(storeName, store);
  const storeVariableName = storeName
    .split('-')
    .map(w => `${w[0]?.toUpperCase()}${w.substring(1)}`)
    .join('');
  const [stores, setStores] = useState<{ [key: string]: GenericStoreType } | null>({
